...
@media(max-width:1300px){
  ...
  .ktr.chetv{width:44%}
  /* Контрольная/игра на ЦЕЛУЮ неделю (chasy>=2) — узкая тонкая плашка той же
     высоты, что и получасовая, читалась как "непонятно, чему отдана целая
     неделя" (владелец 2026-08-22): на десктопе разницу в часах показывает
     ширина колонки, а на мобильном все плашки и так во всю ширину, ширина
     ничего не сообщает. Даём такой плашке высоту и вес блочной карточки. */
  .ktr.polnaya{width:100%;justify-content:center;min-height:64px;padding:14px 16px}
  .ktr.polnaya .k{font-size:17px;text-align:center}
    ...
    for el in ch['potok']:
        ...
        if el['tip'] == 'blok':
            ...
            out.append('<div class="%s" style="grid-column:span %d">'
                       '<span class="k">игра</span></div>' % (kl, span))
    return ''.join(out)
