# Канал исполнителя — lineika-grafy (один заход до конца)
...
Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/materials-wt/lineika-grafy.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/materials/diskmat-57/2026-08-18_logika-goda-7/kod_lineika-grafy.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add lineika-grafy --branch zahod/lineika-grafy, затем чтение захода, затем секция
...
- **МЕСТО РАБОТЫ:** **рабочая папка `../materials-wt/lineika-grafy`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/lineika-grafy`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add lineika-grafy --branch zahod/lineika-grafy
  cd ../materials-wt/lineika-grafy
  ```
...
- **ЗОНА (можно менять):** `diskmat-57/docs/LINEJKA-grafy-metody.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone diskmat-57/docs/LINEJKA-grafy-metody.md --kommitit 'diskmat-57/docs/LINEJKA-grafy-metody.md — единственный файл зоны' --zakryt 'ветку не закрывать: перенос и закрытие делает аналитик при приёмке'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/lineika-grafy`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add diskmat-57/docs/LINEJKA-grafy-metody.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Собрать ОДИН документ `diskmat-57/docs/LINEJKA-grafy-metody.md` — разведку по линейке **ГРАФЫ и МЕТОДЫ**.

**ИСТОЧНИКИ ЛИНЕЙКИ — только эти, всего 7:**
1. `~/Documents/Книги/Графы/графы.pdf` — вып. 02 «Графы», Гуровиц и Ховрина
2. `~/Documents/Книги/Кружки-6-7/10-Turniry.pdf` — вып. 10 «Турниры и таблицы»
3. `~/Documents/Книги/Кружки-6-7/11-Logika.pdf` — вып. 11 «Логические задачи»
4. вып. 14 «Логика для всех: от пиратов до мудрецов» — PDF нет, оглавление в реестре
5. `~/Documents/Книги/Кружки-6-7/21-IndukBF.pdf` — вып. 21 «Индукция без формальностей»
6. `_fond/biblioteka/kanel-belov-kovaldzhi-nestandartnye.pdf` — разделы «Графы», «Инварианты», «Метод крайнего», «Игры», «Процессы и операции», «Обратный ход», «Доказательство от противного», «Раскраски»
7. `~/Documents/Книги/Кружки-6-7/programma-vmsh-57-7klass-2016.md` — «Процессы», «Процессы II», «Играем в игры», «Принцип крайнего»
...
- **Игры** — 3 урока: первые стратегии, анализ с конца, три правила разметки позиций, изоморфизм игр.
- **Разгон** — 3 урока: обратный ход, рыцари и лжецы, отрицание. 15.09 здесь вводится РАЗУМНЫЙ ПЕРЕБОР (Р72, Р80).
- **Блока «графы» у нас нет вовсе.** Слово «граф» в 7 классе не произносится, рисовать можно (Р64): позиции игры — вершины, которые мы красим.
⚠ Методы по Р69 — метки задач, не блоки. Оценка с примером живёт на кружке (Р71). Формальной логики, кванторов и записи нет: логика табличная — истинность в таблице, организованный перебор, отрицание через равносильность. Индукция — тонкой нитью со второго полугодия, без терминологии (Р70).
...
```
python3 -c "import re,sys;t=open('diskmat-57/docs/LINEJKA-grafy-metody.md').read();i=len(re.findall(r'(?m)^## ИСТОЧНИК:',t));n=('## НАЛОЖЕНИЕ' in t);d=('## ДЫРЫ' in t);print('источников',i,'наложение',n,'дыры',d);sys.exit(0 if i>=7 and n and d else 1)"
```
...
```
git --no-optional-locks add -- diskmat-57/docs/LINEJKA-grafy-metody.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- diskmat-57/docs/LINEJKA-grafy-metody.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `diskmat-57/docs/LINEJKA-grafy-metody.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone diskmat-57/docs/LINEJKA-grafy-metody.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/lineika-grafy --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/materials`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/materials-wt/lineika-grafy` плюс `grep` по ЖИВОМУ файлу,
