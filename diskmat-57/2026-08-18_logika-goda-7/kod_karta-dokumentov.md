# Канал исполнителя — karta-dokumentov (один заход до конца)
...
Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/materials-wt/karta-dokumentov.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/materials/diskmat-57/2026-08-18_logika-goda-7/kod_karta-dokumentov.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add karta-dokumentov --branch zahod/karta-dokumentov, затем чтение захода, затем секция
...
- **МЕСТО РАБОТЫ:** **рабочая папка `../materials-wt/karta-dokumentov`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/karta-dokumentov`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add karta-dokumentov --branch zahod/karta-dokumentov
  cd ../materials-wt/karta-dokumentov
  ```
...
- **ЗОНА (можно менять):** `diskmat-57/docs/KARTA-DOKUMENTOV.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone diskmat-57/docs/KARTA-DOKUMENTOV.md --kommitit 'diskmat-57/docs/KARTA-DOKUMENTOV.md — единственный файл зоны' --zakryt 'ветку не закрывать: перенос и закрытие делает аналитик при приёмке'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/karta-dokumentov`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add diskmat-57/docs/KARTA-DOKUMENTOV.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Собрать `diskmat-57/docs/KARTA-DOKUMENTOV.md` — карту всей документации проекта. Владелец: документацию надо выстроить и понять, как её правильно организовать, чтобы там хранить все документы и всю информацию.

**ЧТО ОСМОТРЕТЬ — все `.md` в двух папках, список снимается командой, а не берётся из головы:**
```
# снято на живом дереве 2026-09-14: в docs/ 11 файлов .md, в папке арки 33 файла .md
ls -la diskmat-57/docs/ diskmat-57/2026-08-18_logika-goda-7/
```
🔴 Числа в комментарии — снимок момента сборки; у тебя они могут быть другими, работай по своему выводу и назови расхождение в `## ОТЧЁТ`.

**Разделы документа — ровно четыре:**

1. **`## ЧТО ГДЕ ЛЕЖИТ`** — таблица: файл · сколько строк (`wc -l`) · о чём одной фразой · когда последний раз менялся (`ls -la`) · живой или архивный.
2. **`## ДУБЛИ`** — где одно и то же содержание лежит в двух местах. 🔴 Каждый дубль подтверждается грепом по ОБОИМ файлам, команда приводится в строке. Утверждение о дубле без команды не принимается.
3. **`## ЧЕГО НЕТ`** — какие документы проект подразумевает, но которых на диске нет. Опора — ссылки внутри файлов: если документ ссылается на файл, которого нет, это дыра. Проверять командой `ls`.
4. **`## ПРЕДЛОЖЕНИЕ ПО СТРУКТУРЕ`** — как разложить папку `docs/`: какие разделы, что куда, что архивировать. Это ЕДИНСТВЕННОЕ место захода, где можно предлагать; всё предложенное помечается как предложение, а не как решение.

**ЧЕГО НЕ ДЕЛАТЬ.** Ничего не переносить, не переименовывать и не удалять — только описывать. Не трогать файлы вне зоны.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ) — из корня репозитория:**
```
python3 -c "import re,sys,os;t=open('diskmat-57/docs/KARTA-DOKUMENTOV.md').read();r=[s in t for s in ('## ЧТО ГДЕ ЛЕЖИТ','## ДУБЛИ','## ЧЕГО НЕТ','## ПРЕДЛОЖЕНИЕ ПО СТРУКТУРЕ')];n=len([f for f in os.listdir('diskmat-57/docs') if f.endswith('.md')]);c=sum(1 for f in os.listdir('diskmat-57/docs') if f.endswith('.md') and f in t);print('разделы',r,'файлов docs',n,'из них названо',c);sys.exit(0 if all(r) and c>=n-1 else 1)"
```
→ `rc=0`: все четыре раздела на месте и в карте названы почти все файлы `docs/`. Команда прогоняется на ЖИВОМ файле артефакта и живой папке репозитория — не на фикстуре, фикстуры у этого захода нет вовсе.
🔴 **При сборке 2026-09-14 команда падает `FileNotFoundError` — файла ещё нет.** Ожидаемое красное до работы.

**Отрицательный вердикт несёт ОХВАТ В СЕБЕ:** «дублей не найдено, осмотрено N файлов из N».
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (свежий субагент проверяет по диску: каждый названный файл существует, каждое утверждение о дубле подтверждено грепом по обоим файлам), не перечитывает свою же правку. Доля сплошной выборки: сплошная выборка 100 процентов названных источников. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- diskmat-57/docs/KARTA-DOKUMENTOV.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- diskmat-57/docs/KARTA-DOKUMENTOV.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `diskmat-57/docs/KARTA-DOKUMENTOV.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone diskmat-57/docs/KARTA-DOKUMENTOV.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/karta-dokumentov --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/materials`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/materials-wt/karta-dokumentov` плюс `grep` по ЖИВОМУ файлу,
