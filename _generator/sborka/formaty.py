...
OBYAZATELNYE_POLYA = ("imya", "nazvanie", "zachem", "tip_slaida", "akcent", "minuty",
                       "vazhnost", "tip_verstki", "liniya", "byudzhet_slov")
ZAPOLNIT = "заполнить"

# Заход tipologia-odna-os (2026-08-13): ОДНА ось «что за слайд» вместо двух
# (`tip_idei` здесь + Т1-Т5 в `tipy_slajdov.py` описывали одно и то же). Поле шапки
# — `tip_slaida`, закрытый список значений — Т1..Т5 (раскладка блоков,
# `tipy_slajdov.TIPY`) + Т6 (служебный: обложка/визитка/финал/разделитель —
# состав задаёт геометрия, не блоки) + NE_KLASSIFICIROVAN (временное явное
# значение для карточки, которой на миграции не подошёл ни один тип по составу —
# гейт даёт на нём жёлтое, не красное и не молчание, см. `gejt_kartochki.py`).
#
# ⚠ Список ЗАКРЫТЫЙ, но НЕ ОКОНЧАТЕЛЬНЫЙ (тот же принцип, что был у старого
# `TIPY_IDEI`): типология выведена по одной лекции корпуса (`teorkat-vvedenie/L2`,
# 23 карточки), не по всей математике. Слайд, не ложащийся по СОСТАВУ ни в один
# тип, — повод обсудить РАСШИРЕНИЕ типологии с владельцем (временно —
# NE_KLASSIFICIROVAN), а не тихо подобрать ближайшую полку; сообщение гейта
# обязано говорить именно это.
TIPY_SLAJDA = tuple(sorted(tipy_slajdov.TIPY)) + (tipy_slajdov.TIP_SLUZHEBNYJ,
                                                    tipy_slajdov.NE_KLASSIFICIROVAN)

# Заход tipologia-odna-os: перевод старого `TIPY_IDEI_BEZ_TREBOVANIYA_SVYAZEJ`
# (служебные `divider/cover/vizitka/closing` + `narrative`) на новую ось.
# Служебным (Т6) вводить нечего — у них нет содержания вовсе (та же причина, что
# была у старого списка служебных). Т4/Т5 — оба чисто нарративные раскладки
# (побитово идентичны, см. `tipy_slajdov.py`): вступление пересказывает прошлую
# лекцию, итог собирает уже сказанное — ни то, ни другое не вводит нового понятия
# и не обязано опираться на конкретное. Старый `narrative` механически ложился
# только в Т4 (Т5 на L2 собрал 0 карточек), но семантика — общая для обоих;
