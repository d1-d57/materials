...
# Верх/низ `.zone.copy` — те же именованные числа, что и X_*, а не литерал внутри
# CSS-строки (Д5 дочистки-2: «поле меняется в ОДНОМ месте, смета следует за ним
# автоматически» — то же правило, которое уже применено к X_TEKST, теперь и к Y).
# Низ уравнен с верхом (заход kadr-uzor-i-vmeshchenie, Э3): было 12, обрезало
# формулу на `retrakt` полем, на которое контенту залезать не давали, хотя сам
# он не переполнен. `smeta.py` импортирует оба, литерала 16 там больше нет.
ZONA_PAD_TOP = 4
ZONA_PAD_BOTTOM = 4
...
GLOBAL_CSS = (
    ...
    # Низ уравнен с верхом, не 12px (заход kadr-uzor-i-vmeshchenie, Э3): владелец —
    # «обрезается не потому, что формула большая, а потому что поле введено, на
    # которое формула не может залезть, это надо убрать». Не 0: полный ноль убрал бы
    # последний зазор до необрамлённой кромки зоны на плотных слайдах; centering
