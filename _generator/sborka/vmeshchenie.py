...
# Пол бокса иллюстрации в px. Считается из базы (`korpus`, слайды, где картинка
# РЕАЛЬНО есть) как p5 минус запас, а не из общих соображений: ниже него полоса
# перестаёт быть полосой с чертежом. Держится в px, потому что «видно ли
# картинку» — физический вопрос, а доля от оси зависит (8% ширины = 115px, 8%
# высоты = 65px). Абсолютная страховка — `ILL_FLOOR_ABS_PX`: даже если база
# однажды съедет вниз, солвер не вправе оставить чертежу меньше.
ILL_FLOOR_ABS_PX = 180
...
def pol_boksa_kartinki(corpus, axis):
    """Пол бокса иллюстрации в PX для этой оси — «уже этого чертёж не живёт».

    Считается из базы: p5 боксов тех слайдов, где картинка РЕАЛЬНО есть, минус
    тот же запас 15%, каким живут остальные зоны, и не ниже абсолютной
    страховки `ILL_FLOOR_ABS_PX`. Руками не вписывается (KONSTITUCIYA §10):
    поменяются деки — поменяется и пол."""
    stat = corpus.get("boks_illustracii_%s_px" % axis)
    if not stat:
        return ILL_FLOOR_ABS_PX
    span = max(stat["p95"] - stat["p5"], 0.0)
...
def _potolok_liniya_po_polu_kartinki(corpus, axis):
    ...
    Вертикальная полоса делит ШИРИНУ холста, горизонтальная — ВЫСОТУ, поэтому
    один и тот же физический пол даёт разные проценты; ровно поэтому пол и
    держится в px, а переводится здесь."""
    razmer = korpus.CANVAS_W if axis == "vertical" else korpus.CANVAS_H
    return round(100.0 * (razmer - pol_boksa_kartinki(corpus, axis)) / razmer, 2)
...
def build_zones(corpus=None):
    ...
    # liniya — ДОЛЯ ТЕКСТА, значит её ПОТОЛОК и есть «пол картинки»: чем шире
    # текст, тем уже полоса. Потолок берётся не из общих соображений, а из
    # физического пола бокса (`_potolok_liniya_po_polu_kartinki`) — прямое
    # требование владельца 12.08: «должно быть очень чётко оговорён потолок и
    # пол… который адекватно просто посчитан, не из каких-то общих соображений,
    # а из базы существующих, где мы делали это просто глазами».
    #
    # Горизонтальная: владелец «15–80% широко» + корпус (медиана ≈49%, p95≈69%)
    # согласны в ту же сторону.
    liniya_h = _bounds_from_corpus(c["liniya_equiv_horizontal_pct"], abs_floor=15, abs_ceil=85,
                                    min_owner=15,
                                    max_owner=_potolok_liniya_po_polu_kartinki(c, "horizontal"))
    ...
    # полоса), корпус показывает ОБРАТНОЕ — во всех измеренных вертикальных
    # сплитах текст ШИРОКИЙ, иллюстрация — узкая полоса. Жёсткая зона объединяет
    # ОБЕ гипотезы (не выбирает молча одну), мягкий приоритет (Э2/Э3) тянет к
    # корпусной медиане — «похоже на то, что владелец уже признал хорошим».
    #
    # ⚠ БЫЛО `abs_ceil=92` — и это оказалось дефектом, найденным по прямому
    # вопросу владельца 12.08 («я не очень понимаю, что такое 92 и 82 на
    # разделителе… возможно, картинку не будет видно»). 92 держались ровно на
    # четырёх слайдах Л1 (`s02`/`s09`/`s11`/`s12`), где иллюстрации НЕТ вовсе, а
    # `.rail` — декоративная полоска 116px. Солвер, дойдя до потолка, оставлял
    # живому чертежу 115px из 1440 — прятал картинку, формально не выходя из
    # зоны. `korpus.panelej_v_polose` эти слайды из статистики убрал (p95 упал
    # 92 → 82), потолок теперь физический.
    liniya_v = _bounds_from_corpus(c["liniya_equiv_vertical_pct"], abs_floor=15,
                                    abs_ceil=_potolok_liniya_po_polu_kartinki(c, "vertical"),
                                    min_owner=15,
                                    max_owner=_potolok_liniya_po_polu_kartinki(c, "vertical"))
