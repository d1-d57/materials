...
типов фазы 2 плюс служебный. Т1-Т5 раскладка исходно дословно из
`TIPOLOGIA-SLAJDOV.md` части 3.1 (заход porozhdenie-kartochki), Т1-Т3 РАЗДВИНУТЫ
заходом tipologia-odna-os (2026-08-13, решения владельца на интервью — не
переоткрывать). Т5 «Итог» СЛИТ В Т4 заходом tipologia-dochistka (2026-08-14,
решение владельца на интервью — не переоткрывать): раскладка совпадала с Т4
побитово (см. история ниже), совмещённый тип назван «Нарративный»:
    ...
    | Т4 Нарративный        | Н                              | Н | — | 7 |
    | Т6 Служебный          | (см. ниже — состав не проверяется этим реестром) | — | — | 0 |

в структуру данных, которую `porodit_slaid.py` раскладывает по слотам.

ИСТОРИЯ: Т4 «Разделитель» и Т5 «Итог» несли раскладку, совпадающую ПОБИТОВО (см.
типологию часть 4 п.1) — различался только `nazvanie`. Оба имени были неверны по
существу («Разделитель» уже занят настоящими разделителями разделов в Т6;
«Итог» врёт про слайды в начале/середине лекции) — заход tipologia-dochistka
слил Т5 в Т4 под именем «Нарративный», описывающим СОСТАВ, а не позицию.
...
Раскладки ПЕРЕСЕКАЮТСЯ НАРОЧНО (О·У подходит и Т1, и Т2, У·Д — и Т2, и Т3) — это
НОРМА, не дефект: тип назначается АВТОРОМ на фазе 1/2, а не
...
# Раскладка ТОЛЬКО для порождения карточки (`porodit_slaid.py --tip Т6`) — форма
# идентична бывшей Т4/Т5 (один central+required+multi narrativ-слот). НЕ ключ в
# `TIPY`: гейт (`gejt_kartochki.py`) состав Т6 по-прежнему не сверяет вовсе —
# инвариант «у Т6 нет ключа в TIPY» (докстрока выше) этой константой не нарушен,
# она лишь даёт `porodit_slaid.py` материал для тела карточки (заход
# tipologia-dochistka, Э3 — до него `--tip` Т6 не знал вовсе).
SLOTS_SLUZHEBNYJ = (_slot("narrativ", required=True, multi=True, central=True),)
