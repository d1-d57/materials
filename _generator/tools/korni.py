#!/usr/bin/env python3
"""korni.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/korni.py`.
...
в этот модуль (`import korni`; `korni.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: реестр корней; соседние инструменты импортируют его как библиотеку.
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "korni.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера korni.py не найдена: {ДВЕРЬ}")
