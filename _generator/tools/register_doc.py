#!/usr/bin/env python3
"""register_doc.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/register_doc.py`.
...
в этот модуль (`import register_doc`; `register_doc.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: дверь регистрации `.md` — зовут руками из корня репозитория.
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "register_doc.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера register_doc.py не найдена: {ДВЕРЬ}")
