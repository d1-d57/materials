...
# ── Слепая зона (пункт 4 докстринга) — HANDOFF.md без дефикса НЕ ловится ──
# Не баг, а честно объявленная граница: табличный тест выше уже сверяет
# rc=0, здесь — что причина именно «имя не попало под глоб», а не «файл
# внезапно несёт маркер».
if grep -q 'собран' "$D/HANDOFF.md"; then
  echo "❌ ЛОВУШКА: HANDOFF.md неожиданно несёт маркер — слепая зона перестала демонстрироваться"
  fail=1
else
  echo "  ✅ HANDOFF.md: молчание объясняется именно слепой зоной (маркера и так нет, имя не ловится)"
fi
