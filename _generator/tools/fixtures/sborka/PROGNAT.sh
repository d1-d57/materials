...
echo "── ловушка 44: 🔴 (Д6 дочистки-2 pravila-kadra) ИНВАРИАНТ «СЦЕНА = БЛОК» ЛОВИТ РЕГРЕССИЮ Д4"
...
L44_SRC="$SBORKA"
L44_TMP=$(mktemp -d) || { echo "❌ ЛОВУШКА 44: mktemp не отработал"; exit 1; }
cp "$L44_SRC"/*.py "$L44_TMP/" || { echo "❌ ЛОВУШКА 44: копия sborka/ не собралась"; rm -rf "$L44_TMP"; exit 1; }
# яд: `blocks_for_render` снова отдаёт блоки БЕЗ авто-тегов — состояние Д4 дословно
python3 - "$L44_TMP/bloki.py" <<'PY44' || { echo "❌ ЛОВУШКА 44: не удалось внести поломку"; rm -rf "$L44_TMP"; exit 1; }
import sys
p = sys.argv[1]
s = open(p, encoding="utf-8").read()
marker = "    live = [b for b in blocks if b.telo.strip()]\n    if has_manual_scenes(live):\n        return live\n"
assert s.count(marker) == 1, "якорь поломки не единственен (%d) — ловушка 44 устарела" % s.count(marker)
s = s.replace(marker, "    return [b for b in blocks if b.telo.strip()]\n" + marker)
open(p, "w", encoding="utf-8").write(s)
PY44
OUT44=$(python3 "$L44_TMP/gejt_kartochki.py" "$REPO_ROOT/teorkat-vvedenie/L2" 2>&1)
RC44=$?
rm -rf "$L44_TMP"
[ "$RC44" -ne 0 ] || {
  echo "❌ ЛОВУШКА 44: сцены сломаны нарочно, а гейт дал rc=0 — инвариант «сцена = блок» не краснеет:"
  echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q 'инвариант «сцена = блок» нарушен' || {
  echo "❌ ЛОВУШКА 44: гейт покраснел, но НЕ на инварианте сцен (значит поймал что-то другое):"
  echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q 'СУММА СЦЕН ПО ЛЕКЦИИ:' || {
  echo "❌ ЛОВУШКА 44: сумма сцен по лекции числом не напечатана — падение суммы снова будет невидимо:"
  echo "$OUT44"; exit 1; }
# и обратная половина: на ЗДОРОВОМ генераторе тот же инвариант обязан МОЛЧАТЬ
OUT44OK=$(python3 "$SBORKA/gejt_kartochki.py" "$REPO_ROOT/teorkat-vvedenie/L2" 2>&1)
echo "$OUT44OK" | grep -q 'инвариант «сцена = блок» нарушен' && {
  echo "❌ ЛОВУШКА 44: инвариант краснеет на ЗДОРОВОЙ лекции — гейт, красящий здоровое, обойдут --no-verify:"
  echo "$OUT44OK"; exit 1; }
echo "$OUT44OK" | grep -q 'СУММА СЦЕН ПО ЛЕКЦИИ:' || {
  echo "❌ ЛОВУШКА 44: сумма сцен не печатается на зелёном прогоне (а она обязана печататься ВСЕГДА):"
  echo "$OUT44OK"; exit 1; }
echo "  ✅ ловушка 44: сцены сломаны в копии как в Д4 → гейт КРАСНЫЙ на инварианте; на здоровой лекции молчит; сумма сцен печатается числом в обоих случаях"
