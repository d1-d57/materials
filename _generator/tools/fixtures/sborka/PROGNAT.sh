...
#
# СКОЛЬКО ЛОВУШЕК — ПЕЧАТАЕТ САМ ПРОГОН (шапка числа НЕ хранит, `KONSTITUCIYA
# §10`; тот же приём, что в `fixtures/zahod/PROGNAT.sh`). Хранившееся здесь
# «ДВАДЦАТЬ ЧЕТЫРЕ» уже разъехалось с телом.
# Ловушки 18-24 — регрессионные, все семь предъявлены
...
# Число ловушек — СЧИТАЕТСЯ, а не объявляется (см. шапку).
# `|| true` — не украшение: при `set -e` греп с нулём совпадений (код 1)
...
echo "── ловушка 43: 🔴 заход sceny-iz-blokov — гейты Э2/Э3 УМЕЮТ КРАСНЕТЬ"
# Клауза 3 критерия готовности захода: зелёный гейт, который ни на чём не
# краснеет, — не гейт, а декорация. Фикстура sceny-negativ несёт две карточки,
# каждая нарушает РОВНО одну клаузу (Э3: доказательство в сцене предмета — с
...
OUT43=$(python3 "$SBORKA/gejt_kartochki.py" "$L43" 2>&1) && {
  ...
  echo "$OUT43"; exit 1; }
echo "$OUT43" | grep -q "проверено 2 из 2" || {
  echo "❌ ЛОВУШКА 43: вердикт без охвата «проверено 2 из 2»:"; echo "$OUT43"; exit 1; }
echo "$OUT43" | grep -q "dokazatelstvo-v-scene-predmeta:.*доказательство обязано быть на ДРУГОЙ сцене" || {
  echo "❌ ЛОВУШКА 43: клауза Э3 (доказательство в сцене предмета) не сработала:"
  echo "$OUT43"; exit 1; }
echo "$OUT43" | grep -q "ruchnoj-tag-bez-polya:.*sceny_vruchnuyu.*не заполнено" || {
  echo "❌ ЛОВУШКА 43: клауза Э2 (ручной тег без поля-обоснования) не сработала:"
  echo "$OUT43"; exit 1; }
echo "$OUT43" | grep -q "замечаний 2" || {
  echo "❌ ЛОВУШКА 43: ожидалось РОВНО два замечания (по одному на карточку), сводка другая:"
  echo "$OUT43"; exit 1; }
echo "  ✅ ловушка 43: две негативные фикстуры Э2/Э3 — каждая красит СВОЮ клаузу поимённо, rc≠0, замечаний ровно 2"

echo "── ловушка 44: 🔴 заход kod_rebra-blokov.md, Э2 — адресат рёбер УМЕЕТ КРАСНЕТЬ/ЖЕЛТЕТЬ"
# Клауза 2 критерия готовности захода: граф доказанно умеет краснеть на новых рёбрах —
# половина ЭТОГО инструмента (гейт фазы 2), не graf_zavisimostej.py. Фикстура rebra-negativ
...
OUT44=$(python3 "$SBORKA/gejt_kartochki.py" "$L44" 2>&1) && {
  ...
  echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q "проверено 2 из 2" || {
  echo "❌ ЛОВУШКА 44: вердикт без охвата «проверено 2 из 2»:"; echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q "dokazatelstvo-bez-adresata: блок \[dokazatelstvo\] без 'dokazatelstvo_opiraetsya_na'" || {
  echo "❌ ЛОВУШКА 44: красная клауза «доказательство без адресата» не сработала:"
  echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q "замечаний 1" || {
  echo "❌ ЛОВУШКА 44: ожидалось РОВНО одно красное замечание (примеру — жёлтое, не красное):"
  echo "$OUT44"; exit 1; }
echo "$OUT44" | grep -q "primer-bez-adresata: блок \[primer\] без 'primer_dlya'" || {
  echo "❌ ЛОВУШКА 44: жёлтая клауза «пример без адресата» не сработала:"
  echo "$OUT44"; exit 1; }
echo "  ✅ ловушка 44: доказательство без адресата — красное, пример без адресата — жёлтое, rc≠0"

# 🔴 ДВЕ РАЗНЫЕ ЛОВУШКИ ПОЛУЧИЛИ ОДИН НОМЕР В ПАРАЛЛЕЛЬНЫХ ВЕТКАХ, И ОБЕ ЖИВЫЕ
# (слияние zahod/pravila-kadra в основную, 2026-08-15). Ловушка 44 выше —
# заход rebra-blokov, Э2: адресат рёбер умеет краснеть/желтеть. Ловушка
# 44-бис ниже — заход pravila-kadra, Д6: инвариант «сцена = блок» ловит
# регрессию Д4. Общего у них только номер. Взять сторону целиком значило бы
# снести чужой сторож молча — ровно тот класс потери, где выпавшая ловушка
# неотличима от никогда не написанной, поэтому ветке дан свой номер и свои
# имена переменных (L44B_*, OUT44B, RC44B).
echo "── ловушка 44-бис: 🔴 (Д6 дочистки-2 pravila-kadra) ИНВАРИАНТ «СЦЕНА = БЛОК» ЛОВИТ РЕГРЕССИЮ Д4"
# Регрессия Д4: `bloki.blocks_for_render` перестала авто-тегировать сцены, рендер
# дека получил тела без единого `{@N}`, 14 слайдов из 26 схлопнулись в ОДНУ сцену
# (сумма 50 → 28) — и ВСЕ гейты остались зелёными. Ловушка воспроизводит ту самую
# поломку в ВРЕМЕННОЙ КОПИИ `sborka/` и требует, чтобы гейт покраснел.
#
# 🔴 ПОЧЕМУ ПОЛОМКА КОДА, А НЕ КРИВАЯ КАРТОЧКА. Карточкой этот дефект не выражается
# вовсе: в авто-режиме сцены равны блокам ПО ПОСТРОЕНИЮ, и никакая разметка автора
# их не разведёт. Д4 сломался в генераторе, значит и ловушка обязана ломать генератор
# — иначе она сторожит не то, что упало. Правится файл ТОЛЬКО во временной копии,
# рабочее дерево не трогается ни на байт.
L44B_SRC="$SBORKA"
L44B_TMP=$(mktemp -d) || { echo "❌ ЛОВУШКА 44-бис: mktemp не отработал"; exit 1; }
cp "$L44B_SRC"/*.py "$L44B_TMP/" || { echo "❌ ЛОВУШКА 44-бис: копия sborka/ не собралась"; rm -rf "$L44B_TMP"; exit 1; }
# яд: `blocks_for_render` снова отдаёт блоки БЕЗ авто-тегов — состояние Д4 дословно
python3 - "$L44B_TMP/bloki.py" <<'PY44B' || { echo "❌ ЛОВУШКА 44-бис: не удалось внести поломку"; rm -rf "$L44B_TMP"; exit 1; }
import sys
p = sys.argv[1]
s = open(p, encoding="utf-8").read()
marker = "    live = [b for b in blocks if b.telo.strip()]\n    if has_manual_scenes(live):\n        return live\n"
assert s.count(marker) == 1, "якорь поломки не единственен (%d) — ловушка 44-бис устарела" % s.count(marker)
...
[ "$RC44B" -ne 0 ] || {
  ...
  echo "$OUT44B"; exit 1; }
echo "$OUT44B" | grep -q 'инвариант «сцена = блок» нарушен' || {
  echo "❌ ЛОВУШКА 44-бис: гейт покраснел, но НЕ на инварианте сцен (значит поймал что-то другое):"
  echo "$OUT44B"; exit 1; }
echo "$OUT44B" | grep -q 'СУММА СЦЕН ПО ЛЕКЦИИ:' || {
  echo "❌ ЛОВУШКА 44-бис: сумма сцен по лекции числом не напечатана — падение суммы снова будет невидимо:"
  echo "$OUT44B"; exit 1; }
# и обратная половина: на ЗДОРОВОМ генераторе тот же инвариант обязан МОЛЧАТЬ
OUT44BBOK=$(python3 "$SBORKA/gejt_kartochki.py" "$REPO_ROOT/teorkat-vvedenie/L2" 2>&1)
echo "$OUT44BBOK" | grep -q 'инвариант «сцена = блок» нарушен' && {
  echo "❌ ЛОВУШКА 44-бис: инвариант краснеет на ЗДОРОВОЙ лекции — гейт, красящий здоровое, обойдут --no-verify:"
  echo "$OUT44BBOK"; exit 1; }
echo "$OUT44BBOK" | grep -q 'СУММА СЦЕН ПО ЛЕКЦИИ:' || {
  echo "❌ ЛОВУШКА 44-бис: сумма сцен не печатается на зелёном прогоне (а она обязана печататься ВСЕГДА):"
  echo "$OUT44BBOK"; exit 1; }
echo "  ✅ ловушка 44-бис: сцены сломаны в копии как в Д4 → гейт КРАСНЫЙ на инварианте; на здоровой лекции молчит; сумма сцен печатается числом в обоих случаях"
