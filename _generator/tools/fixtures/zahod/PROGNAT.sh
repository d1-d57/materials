...
#
# ЛОВУШЕК ТРИДЦАТЬ ПЯТЬ (задание требовало не менее девяти):
...
for LIVE in "$LIVE1" "$LIVE2"; do
  ...
  KROME=$(echo "$OUT" | grep -v '^── ' | grep -v 'З10' | grep -v 'З11' | grep -v 'З12' | grep -v '^[[:space:]]*$' || true)
  [ -z "$KROME" ] || {
    echo "❌ ЛОВУШКА 1: линтер НЕ МОЛЧИТ (сверх объявленных З10/З11/З12) на признанном хорошим живом заходе — $LIVE. Вывод:"
    echo "$KROME"; exit 1; }
done
echo "  ✅ ловушка 1: оба живых калибровочных захода — молчат по всем проверкам, кроме объявленных З10/З11/З12"
