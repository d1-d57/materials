...
# 🔴 ОДНОРАЗОВЫЙ КАТАЛОГ, А НЕ ОТСЛЕЖИВАЕМАЯ ПАПКА. Раньше здесь стояло
# `Z=_generator/tools/fixtures/uroki/zakrytie`, и строка 50 штамповала в
# `$Z/DOLG.md` СЕГОДНЯШНЮЮ дату — то есть каждый прогон регресса пачкал рабочую
# зону и ронял Г1 (`зона доехала в git`) у того, кто регресс прогнал. Дефект
# маскируется ровно один день: в дереве лежит дата последнего коммита фикстуры,
# и в этот день перезапись совпадает байт в байт. Нарушало собственное правило
# семьи — «гейт не смеет писать в проверяемый репозиторий» (докстринг
# `check_tool_contract.py`, урок 1.7). Отслеживаемая `zakrytie/` больше не
# используется и остаётся нетронутой: удалять историю фикстуры — не наше дело.
Z=$(mktemp -d)/zakrytie
trap 'rm -rf "$(dirname "$Z")"' EXIT
mkdir -p "$Z/arka"
cat > "$Z/DOLG.md" <<'MD'
