...
#
# СКОЛЬКО ЗДЕСЬ ЛОВУШЕК — НЕ ВПИСАНО СЛОВОМ И НЕ ДОЛЖНО БЫТЬ (`KONSTITUCIYA §10`).
# Число считает команда, и она же печатается прогоном первой строкой:
#     grep -c '^# ── ЛОВУШКА' _generator/tools/fixtures/korni/PROGNAT.sh
# *Цена, из-за которой правило применено сюда (дата данных 2026-08-15): в этой
# строке стояло число СЛОВОМ, и оно разошлось с файлом — ловушки `1-мутация`,
# `6-бис`, `7-бис МУТАЦИЯ` и `9` дописывались позже, шапку никто не правил.
# Читатель, поверивший слову, недосчитался проверок и не знал бы, что их
# молчание вообще что-то значит. Сколько их сейчас — печатает прогон, выше.*
#
# ПУТЕВОДИТЕЛЬ ПО ЛОВУШКАМ (перечень, а НЕ счётчик — счётчик выше), каждая
# проверена МУТАЦИЕЙ (гейт, который не умеет падать, — театр):
...
#   9. дом шаблонов — репозиторий ИНСТРУМЕНТОВ, а не `GIT_ZONA_REPO`.
# Строки-мутации (`1-мутация`, `6-бис`, `7-бис МУТАЦИЯ`) — тоже ловушки и тоже
# считаются командой: они и есть доказательство, что соседняя умеет падать.
#
...
# 🔴 ЧИСЛО ЛОВУШЕК — СЧИТАЕТСЯ, А НЕ ОБЪЯВЛЯЕТСЯ (`KONSTITUCIYA §10`). Печатаем
# его прогоном, чтобы шапка физически не могла разойтись с файлом: разойтись
# теперь нечему — в шапке числа нет, есть эта команда.
LOVUSHEK=$(grep -c '^# ── ЛОВУШКА' "$SAM")
echo "ловушек в файле: $LOVUSHEK (счёт командой: grep -c '^# ── ЛОВУШКА')"
