...
#
# ⚠ НАЗВАННЫЙ ЗАЗОР (дата данных 2026-07-30). Хук поднимет эту фикстуру только на
# `bootstrap_zahod.py`. Остальные четыре имени объявлены ВЕРНО и всё равно не
# сработают: `.githooks/pre-commit:62-63` отбирает staged-пути грепом
# `^_generator/tools/.*\.py$` · `^_generator/tools/fixtures/.*\.sh$` · `^\.githooks/`,
# а `skeleton/engine.js`, `skeleton/base.css`, `build_deck.py` и `render.py` не
# подходят ни под один шаблон — объявление читается только для путей, УЖЕ прошедших
# этот фильтр (`check_tool_contract.py:107-120`, далее `--fixtures-for`). Лечится
# одной строкой в хуке; хук был запрещён заходу поимённо, поэтому здесь только
# диагноз. Заработает в ту секунду, когда строку добавят — имена уже на месте.
#
...
#
# ЧЕТЫРЕ ЛОВУШКИ, каждая оплаченная:
...
if python3 -c "from playwright.sync_api import sync_playwright" > /dev/null 2>&1; then
    ...
    echo "  ⚠ ЛОВУШКИ 1-2 НЕ ПРОВЕРЕНЫ: нет playwright. Ядро захода (движок и"
    echo "    каскад) осталось БЕЗ гейта — поставь \`pip install playwright\`."
...
# ── ЛОВУШКА 3: --worktree кладёт заход ТУДА, КУДА отправляет исполнителя ──────
# Гоняется на ОДНОРАЗОВОМ репо в /tmp: `worktree add` пишет в .git, и делать это
# в боевом репозитории гейт не смеет (урок 1.7).
podgotovit() {
    R="$1"
    mkdir -p "$R/_studio/docs" "$R/_studio/zhurnal/arka" "$R/_generator/tools"
    printf '%s\n' '{{ТЕМА}} {{МОДЕЛЬ}} {{ВЕТКА}}' 'ЗОНА: {{ЗОНА}}' \
...
GIT_ZONA_REPO="$R1" python3 "$T/zvat.py" "$GEN" "$R1" \
    ...
    --opisanie "проба обычного режима" > "$T/plain.txt" 2>&1 || true
...
GIT_ZONA_REPO="$R2" python3 "$T/zvat.py" "$GEN" "$R2" \
    ...
    --worktree probnaya > "$T/wt.txt" 2>&1 || true
...
if [ -d "$WT" ]; then
    [ -f "$WT/_studio/zhurnal/arka/kod_s-wt.md" ] \
        && echo "  ✅ С --worktree заход лёг В РАБОЧУЮ ПАПКУ (дефект закрыт)" \
        || { echo "  ❌ ЗАХОДА НЕТ В РАБОЧЕЙ ПАПКЕ — вернулся дефект: исполнителя"; \
             echo "     отправляют в worktree, а файл пишут в основную папку"; FAIL=1; }

    [ ! -f "$R2/_studio/zhurnal/arka/kod_s-wt.md" ] \
        && echo "  ✅ С --worktree в основном дереве копии НЕ появилось (нет тени)" \
        || { echo "  ❌ ДВЕ КОПИИ ЗАХОДА — ровно та тень, из которой расходятся версии"; FAIL=1; }

    # Путь в стартовой команде обязан вести в ТУ ЖЕ папку, где файл лежит
    V=$(grep -c "cd $WT" "$T/wt.txt" || true)
    [ "$V" -ge 1 ] && echo "  ✅ стартовая команда отправляет в папку, где заход ЛЕЖИТ" \
                   || { echo "  ❌ стартовая команда ведёт не туда, где файл"; FAIL=1; }

    # Зона — РЕПО-ОТНОСИТЕЛЬНАЯ в обоих режимах: это git-путь, а не путь машины
    V=$(grep -c 'ЗОНА: _generator/ _studio/zhurnal/arka/kod_s-wt.md' \
            "$WT/_studio/zhurnal/arka/kod_s-wt.md" || true)
    [ "$V" = "1" ] && echo "  ✅ зона осталась репо-относительной (заход входит в свою зону)" \
                   || { echo "  ❌ зона не репо-относительная — заход не закоммитит сам себя"; FAIL=1; }

    V=$(grep -c 'kod_s-wt.md' "$WT/_studio/docs/KARTA.md" || true)
    [ "$V" = "1" ] && echo "  ✅ регистрация ушла в KARTA.md РАБОЧЕЙ папки (её и коммитит исполнитель)" \
                   || { echo "  ❌ регистрации нет в KARTA.md рабочей папки — документ уедет сиротой"; FAIL=1; }
fi
...
if [ "$BROWSER_OK" = "1" ]; then
    echo "Охват: ловушки 1-4 проверены, 4 из 4 (браузер поднялся)"
else
    echo "Охват: проверены ловушки 3-4, 2 из 4 — ЛОВУШКИ 1-2 ПРОПУЩЕНЫ (нет playwright)"
fi
