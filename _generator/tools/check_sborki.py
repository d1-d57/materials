#!/usr/bin/env python3
"""check_sborki.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/check_sborki.py`.
...
в этот модуль (`import check_sborki`; `check_sborki.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: гейт сборки файла-захода — зовут руками и из чужих проектов.
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "check_sborki.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера check_sborki.py не найдена: {ДВЕРЬ}")
