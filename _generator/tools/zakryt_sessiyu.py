#!/usr/bin/env python3
"""zakryt_sessiyu.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
Живой экземпляр — `disciplina/_generator/tools/zakryt_sessiyu.py`. Путь остаётся
...
импорте все имена двери переезжают в этот модуль (`import zakryt_sessiyu`;
`zakryt_sessiyu.<имя>` продолжает работать). Дверь не найдена — внятный отказ,
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "zakryt_sessiyu.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера zakryt_sessiyu.py не найдена: {ДВЕРЬ}")
