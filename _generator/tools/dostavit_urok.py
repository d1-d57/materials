#!/usr/bin/env python3
"""dostavit_urok.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
Живой экземпляр — `disciplina/_generator/tools/dostavit_urok.py`. Путь остаётся
...
импорте все имена двери переезжают в этот модуль (`import dostavit_urok`;
`dostavit_urok.<имя>` продолжает работать). Дверь не найдена — внятный отказ,
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "dostavit_urok.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера dostavit_urok.py не найдена: {ДВЕРЬ}")
