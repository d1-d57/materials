#!/usr/bin/env python3
"""check_ves.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/check_ves.py`.
...
в этот модуль (`import check_ves`; `check_ves.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: хук `.githooks/pre-commit` этого репозитория зовёт `check_ves.py --staged`;
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "check_ves.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера check_ves.py не найдена: {ДВЕРЬ}")
