#!/usr/bin/env python3
"""schet_nezakrytogo.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/schet_nezakrytogo.py`.
...
в этот модуль (`import schet_nezakrytogo`; `schet_nezakrytogo.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: первый ход каждого захода — зовут руками.
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "schet_nezakrytogo.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера schet_nezakrytogo.py не найдена: {ДВЕРЬ}")
