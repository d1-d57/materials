#!/usr/bin/env python3
"""bootstrap_zahod.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
оригиналом в `disciplina/_generator/tools/bootstrap_zahod.py` молча: в
`disciplina` он переписан (дверь интервью `--intervyu`/`--finalizirovano`,
блоки `verifikator_blok`/`warning_blok`), а в `materials` осталась прежняя
...
`if __name__ == "__main__":`) ЭТОГО НЕ ПОКРЫВАЕТ. Фикстуры
(`fixtures/nevlitye/PROGNAT.sh`, `fixtures/gigiena/PROGNAT.sh`,
`fixtures/dvizhok/PROGNAT.sh`) делают `import bootstrap_zahod as bz` и зовут
его функции напрямую. Наивный диспетчер под `__main__`-гвардом ничего не
определяет при обычном импорте: `AttributeError` на каждом таком прогоне.
Найдено при проверке того же дефекта на `priyomka.py` (там он ломает не
тест, а живой прод-хук `.githooks/pre-commit` через `check_faza_priyomki.py`)
— применено сюда тем же приёмом заранее, а не постфактум.
...
сам — `"__main__"`, когда файл ЗАПУЩЕН (`python3 bootstrap_zahod.py …`), и
`"bootstrap_zahod"`, когда он ИМПОРТИРОВАН (`import bootstrap_zahod`).
...
"bootstrap_zahod"`, её `__main__`-блок НЕ срабатывает (не запускает `main()`,
не делает `sys.exit`), а все её имена достаются в `_ns` и переносятся в этот
модуль: `import bootstrap_zahod as bz; bz.<что угодно>` продолжает работать.
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "bootstrap_zahod.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера bootstrap_zahod.py не найдена: {ДВЕРЬ}")
