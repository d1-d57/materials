#!/usr/bin/env python3
"""dolg_repozitoriev.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.

ОТКЛЮЧЕНО 2026-08-22, заход `vynos-instrumentov`: копия совпадала с живым экземпляром байт в байт (сторож `sverka_kopij.py`: необъявленных расхождений не было) —
чинить в двух местах больше нечего; единственный экземпляр теперь там.
Живой экземпляр — `disciplina/_generator/tools/dolg_repozitoriev.py`. Путь остаётся
...
импорте все имена двери переезжают в этот модуль (`import dolg_repozitoriev`;
`dolg_repozitoriev.<имя>` продолжает работать). Дверь не найдена — внятный отказ,
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "dolg_repozitoriev.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера dolg_repozitoriev.py не найдена: {ДВЕРЬ}")
