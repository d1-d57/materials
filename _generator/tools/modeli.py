#!/usr/bin/env python3
"""modeli.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
Живой экземпляр — `disciplina/_generator/tools/modeli.py`. Путь остаётся
...
импорте все имена двери переезжают в этот модуль (`import modeli`;
`modeli.<имя>` продолжает работать). Дверь не найдена — внятный отказ,
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "modeli.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера modeli.py не найдена: {ДВЕРЬ}")
