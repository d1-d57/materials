...
`check_kartoteka.py`). Их четыре:
  ...
  4. признак «заход/хэндофф» — эвристика по ФИКСИРОВАННОМУ списку имён и
     трём content-префиксам, не по смыслу. Рукописный файл с нестандартным
     именем (`HANDOFF.md` без дефиса, `peredacha-*.md` строчными, суффиксный
     `...-peredacha.md` — все три реально существуют в дереве) или с шапкой,
     набранной обычным дефисом вместо «—» (U+2014), под признак не подпадёт
     и пройдёт молча, даже будучи заходом/хэндоффом по сути. Список имён —
     тот, что назвал заход `kod_zapret.md`; расширять его в одиночку не моё
     решение (см. `## ВОПРОСЫ` того же файла).
...
NAME_GLOBS = ("kod_*.md", "HANDOFF-*.md", "PEREDACHA-*.md", "ZAHOD-*.md")
...
def declares_itself(path):
    ...
    if any(fnmatch.fnmatchcase(path.name, g) for g in NAME_GLOBS):
        return True
    try:
