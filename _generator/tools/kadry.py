...
def summa_scen(slaidy):
    """Сумма `data-scenes` по деке и число слайдов, схлопнутых в одну сцену —
    Д4 дочистки-2 захода pravila-kadra.

    🔴 ЗАЧЕМ ЭТО В СКРИНШОТЕРЕ. Регрессия сцен (сумма 50 → 28, четырнадцать
    слайдов из двадцати шести схлопнулись в одну сцену) прошла И приёмку,
    И ПОСЛЕ-верификатора: оба смотрели на текст и на кадры, а сцены живут во
    времени и в атрибутах — на статичном кадре их отсутствие не видно вообще
    никак. Кадр показывает ПОСЛЕДНЮЮ сцену, и слайд, где всё вылезло разом,
    выглядит на нём в точности как здоровый.

    Гейт карточек (`gejt_kartochki.py`, Д6) проверяет ту же величину ПО
    ИСХОДНИКУ; здесь она считается ПО СОБРАННОЙ ДЕКЕ — по тому же файлу, что
    откроет владелец. Две разные точки замера на одну величину заведены нарочно:
    Д4 сломался ровно между ними (карточки были в порядке, рендер — нет)."""
    scen = sum(s["scenes"] for s in slaidy)
    odna = [s["id"] for s in slaidy if s["scenes"] == 1]
    return scen, odna
...
def main():
    ...
    ap.add_argument("--scen-ne-menshe", type=int, metavar="N",
                     help="Д4: КРАСНЕТЬ (rc=1), если сумма сцен по собранной деке упала ниже N. "
                          "Число берут из прошлого прогона — сумма печатается всегда, и без "
                          "флага тоже; флаг превращает наблюдение в гейт для CI/приёмки")
    args = ap.parse_args()
    ...
    if not slaidy:
        ...
        return 1
    # Д4: сумма сцен печатается ДО скриншотов и БЕЗУСЛОВНО — даже когда отбор
    # характерных слайдов ниже провалится. Величина, которую печатают только
    # в успешном случае, пропадает ровно тогда, когда она нужнее всего.
    scen, odna = summa_scen(slaidy)
    print("СЦЕНЫ ПО ДЕКЕ: сумма %d, слайдов %d, из них односценовых %d%s"
          % (scen, len(slaidy), len(odna), (": " + ", ".join(odna)) if odna else ""))
    if args.scen_ne_menshe is not None and scen < args.scen_ne_menshe:
        print("❌ СЦЕНЫ ПОТЕРЯНЫ: сумма %d, а требовалось не меньше %d — на кадрах этого "
              "НЕ ВИДНО (кадр показывает последнюю сцену, схлопнутый слайд выглядит "
              "как здоровый), поэтому проверка числом (Д4 pravila-kadra)"
              % (scen, args.scen_ne_menshe), file=sys.stderr)
        return 1
