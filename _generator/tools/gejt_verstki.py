...
Гейт НЕ встроен в сборку (`deck.py`) — вызывается отдельной командой; живая точка вызова —
`_generator/tools/fixtures/verstki/PROGNAT.sh` (маркер `TOOL-CONTRACT-COVERS`, тот же механизм,
которым уже пользуется `fixtures/illyustracii/PROGNAT.sh`) и строка в lifecycle-блоке «ЧТО ДАЛЬШЕ»
карточек — та строка принадлежит `bootstrap_lekcii.py`, чужой зоне (см. отчёт захода).
"""
import re
import sys
