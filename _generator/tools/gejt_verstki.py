#!/usr/bin/env python3
"""gejt_verstki.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.

ОТКЛЮЧЕНО 2026-08-22, заход `vynos-instrumentov`: тело переехало туда целиком, байт в байт, вместе со спутник-фикстурой
`fixtures/verstki/PROGNAT.sh`; инструмент судит только переданные ему пути.
Живой экземпляр — `disciplina/_generator/tools/gejt_verstki.py`. Путь остаётся
...
импорте все имена двери переезжают в этот модуль (`import gejt_verstki`;
`gejt_verstki.<имя>` продолжает работать). Дверь не найдена — внятный отказ,
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "gejt_verstki.py"
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера gejt_verstki.py не найдена: {ДВЕРЬ}")
