...
# `_generator/sborka/**` и потому этим заходом не поставлена. Поставят её —
# маркер можно снять: охват `check_tool_contract.has_live_trigger()` тем же
# заходом расширен на `_generator/sborka/*.py` и строку увидит. До этого момента
# маркер — единственная честная форма: без него гейт краснеет, а с ним хотя бы
# названа фаза, на которой его обязаны звать.
