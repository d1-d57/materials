...
# ── Кириллический диапазон в regex (КРТ-033, КРТ-РАЗН-21) ─────────────────────
# 🔴 САМ ДИАПАЗОН СОБИРАЕТСЯ ИЗ ПОЛОВИНОК, А НЕ ПИШЕТСЯ ЦЕЛИКОМ, И ЭТО НЕ
# УКРАШЕНИЕ: буквы отдельно, скобки отдельно — в исходном тексте файла строки
# `[<буква>-<буква>]` не возникает ни разу, и линтер не краснеет на собственном
# списке запрещённых конструкций. Ровно та же причина, по которой из половинок
# склеен `GNU_PAIRS` выше (ошибка уже ловилась ловушкой 7 фикстуры git_zona).
KIR = "А-Яа-яЁё"  # буквы и дефисы — БЕЗ скобок, класс собирается ниже
# Диапазон ВНУТРИ скобочного класса: `[<буква>-<буква>]`. Одиночная буква в
# классе (`[абв]`) безопасна в любой локали — перечисление, а не диапазон, и
# ловить его значило бы красить здоровое.
KIR_RANGE_RE = re.compile(r"\[[^\]\n]*[" + KIR + r"]\s*-\s*[" + KIR + r"][^\]\n]*\]")
# Инструменты, у которых regex разбирает ШЕЛЛ (POSIX-класс, зависящий от locale),
# а не python. `git` не взят: `git grep` встречается редко, а ложное красное на
# каждом списке `["git", …]` дороже пропуска.
SHELL_TOOLS = {"grep", "egrep", "fgrep", "sed", "awk", "gawk", "ugrep"}
SHELL_TOOL_RE = re.compile(r"\b(?:" + "|".join(sorted(SHELL_TOOLS)) + r")\b")
...
def check_cyrillic_range(path, text, tree, ctx):
    """Кластер B, записи `zahody` КРТ-033 и КРТ-РАЗН-21. Кириллический диапазон
    в regex, отданном ШЕЛЛУ, даёт ложный результат или падение — смотря по
    locale; в питоновском `re` он корректен, и красить его нельзя.

    ЗАМЕР 15.08 на машине владельца, `/usr/bin/grep`, файл «Привет / мир /
    ясно / zzz» (эталон — `python3 -c "re.search"`, одна строка):
        LC_ALL=C        grep -cE '<диапазон А-Я>'  →  3   ЛОЖЬ (побайтовый матч)
        LC_ALL=C.UTF-8  то же                      →  1   верно
        LC_ALL=ru_RU.UTF-8 то же                   →  1   верно
    На машине приёмки тот же вызов под `C.UTF-8` не соврал, а УПАЛ — «Invalid
    collation character» (GNU grep). То есть исход шелл-диапазона зависит от
    среды и меняется между владельцем и песочницей, а питоновский `re` даёт
    один и тот же верный ответ везде.

    🔴 РАЗЛИЧЕНИЕ ДВУХ СЛУЧАЕВ — ВЕСЬ СМЫСЛ ПРОВЕРКИ, А НЕ ЕЁ ТОНКОСТЬ. Сторож,
    красящий ЛЮБОЙ кириллический диапазон, покрасит каждый питоновский regex в
    репозитории, где вся проза русская, — и его отключат в тот же день
    (`KONSTITUCIYA §11а`, тот же урок, что уже оплачен сужением GNU-списка).
    Поэтому шелл-контекст назван поимённо: `.sh` целиком, строка, отданная
    шеллу (`os.system`, `shell=True`), список аргументов, зовущий
    `grep`/`sed`/`awk`, и команда, НАПЕЧАТАННАЯ владельцу, — он исполнит её у
    себя в zsh (канон §0, тот же класс, что у `check_shell_to_owner`).

    🔴 ОБЪЯВЛЕННАЯ СЛЕПАЯ ЗОНА: строка `.sh`, в которой есть `python`, не
    судится вовсе. В фикстурах питоновский однострочник живёт ВНУТРИ шелла
    (`python3 -c "…"`) — и это как раз ЗДОРОВАЯ форма, ради которой проверка и
    различает случаи. Цена сужения названа прямо: кириллический диапазон,
    отданный `grep` в одной строке с вызовом python, проверка пропустит.
    """
    bad = []
    if tree is None:
        for n, ln in code_lines(path, text, tree):
            if "python" in ln:
                continue
            m = KIR_RANGE_RE.search(ln)
            if m:
                bad.append((n, m.group(0)))
        return _kir_soobshcheniya(bad)
    for s, n in shell_commands(tree):
        m = KIR_RANGE_RE.search(s)
        if m:
            bad.append((n, m.group(0)))
    for node, vals in arglists(tree):
        if not (SHELL_TOOLS & {v.split("/")[-1] for v in vals}):
            continue
        for v in vals:
            m = KIR_RANGE_RE.search(v)
            if m:
                bad.append((node.lineno, m.group(0)))
    for s, n in printed_strings(tree):
        if not SHELL_TOOL_RE.search(s):
            continue
        m = KIR_RANGE_RE.search(s)
        if m:
            bad.append((n, m.group(0)))
...
def _kir_soobshcheniya(bad):
    return [f"строка {n}: КИРИЛЛИЧЕСКИЙ ДИАПАЗОН «{s}» в шелл-контексте — под "
            f"`LC_ALL=C` матчит побайтово и врёт (замер 15.08: 3 строки вместо "
            f"одной), под UTF-8-локалью GNU grep падает «Invalid collation "
            f"character». Перечисляй буквы явно, либо считай питоном: "
            f"`python3 -c \"import re; …\"` — в `re` тот же диапазон корректен "
            f"и от locale не зависит" for n, s in sorted(set(bad))]
...
CHECKS = [
    ...
    # 🔴 ИМЯ КЛАСТЕРА ТО ЖЕ («переносимость») НАМЕРЕННО, И ЭТО НЕСУЩАЯ ДЕТАЛЬ:
    # `judge()` пускает на не-`.py` файлы РОВНО проверки с этим именем. Назови
    # проверку иначе — и она перестанет видеть `.sh`, то есть тот самый шелл,
    # ради которого заведена.
    ("переносимость", check_cyrillic_range),
