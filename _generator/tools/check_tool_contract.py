#!/usr/bin/env python3
"""check_tool_contract.py — тонкий диспетчер: ОДНА дверь, а не вторая копия.
...
`disciplina`. Живой экземпляр — `disciplina/_generator/tools/check_tool_contract.py`.
...
в этот модуль (`import check_tool_contract`; `check_tool_contract.<имя>` продолжает работать). Дверь не
...
# TOOL-CONTRACT: called-by-hand — живая точка вызова названа: хук `.githooks/pre-commit` этого репозитория зовёт `check_tool_contract.py --staged` и `--fixtures-for`;
...
ДВЕРЬ = КОРЕНЬ_MATERIALS.parent / "disciplina" / "_generator" / "tools" / "check_tool_contract.py"

os.environ.setdefault("GIT_ZONA_REPO", str(КОРЕНЬ_MATERIALS))
# TOOL_CONTRACT_HOME — вторая дверь этого тела (дом фикстур и BASELINE, тело
# слушает именно её, не GIT_ZONA_REPO): без умолчания --fixtures-for сканировал
# бы fixtures/ соседнего репозитория и не нашёл бы здешний охват.
os.environ.setdefault("TOOL_CONTRACT_HOME",
                      str(КОРЕНЬ_MATERIALS / "_generator" / "tools"))
...
if not ДВЕРЬ.is_file():
    ...
    if __name__ == "__main__":
        sys.exit(1)
    raise ModuleNotFoundError(f"дверь диспетчера check_tool_contract.py не найдена: {ДВЕРЬ}")
