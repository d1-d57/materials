...
_PREDLOGI = "аиквосуяАИКВОСУЯ"
# Единицы — куратор-список, не общее «число+любое слово» (заход pravila-kadra,
# Э5): общее правило ловит и ложные случаи (нумерованный пункт «1 текст» без
# точки, обычное числительное перед случайным словом), а надёжно сделать не
# удалось — не делаю его, см. отчёт. Список — то, что реально встречается в
# текстах этого корпуса (математика/лекции): единицы длины/массы/времени,
# «лет»/«года»/«века», порядки величин, проценты и градус.
_EDINICY = (r"см|мм|км|кг|г|л|мин|сек|ч|стр|шт"
            r"|лет|года?|век(?:а|ов)?|тыс|млн|млрд")
_EDINICY_SIMVOL = r"%|°"  # без `\b`: не буквы, после них перед ними \b не срабатывает
_NBSP = " "


def _nerazryvnosti(text):
    """Типографские неразрывные пробелы (заход pravila-kadra, Э5) — единственная
    точка, где текст карточек становится HTML, значит и единственная, где это
    можно сделать один раз на весь пайплайн. Зовётся ПОСЛЕ стеша формул (`$…$`
    заменены на `\\x00M%d\\x00`) — внутрь математики эти замены попасть не могут
    физически, плейсхолдер непрозрачен для всех правил ниже.
    Список (по разговору, дан в отчёте дословно тем же списком):
      · перед закрывающей «»» — не даёт кавычке уехать на новую строку одной;
      · после открывающей «« — не даёт первому слову оторваться от кавычки;
      · после однобуквенных предлогов/союзов (а и к в о с у я) — не даёт им
        повиснуть одни в конце строки;
      · перед тире «—» — не даёт тире начать строку без слова перед ним;
      · между числом и единицей — курируемый список (см. `_EDINICY` и его
        комментарий: общее правило «число+слово» не сделано надёжно нарочно)."""
    text = re.sub(r" +(?=»)", _NBSP, text)
    text = re.sub(r"(?<=«) +", _NBSP, text)
    text = re.sub(r"(?:^|(?<=\s))([%s]) " % _PREDLOGI, r"\1" + _NBSP, text)
    text = re.sub(r" +(?=—)", _NBSP, text)
    text = re.sub(r"(?<=\d) +(?=(?:%s)\b)" % _EDINICY, _NBSP, text)
    text = re.sub(r"(?<=\d) +(?=(?:%s))" % _EDINICY_SIMVOL, _NBSP, text)
    return text
