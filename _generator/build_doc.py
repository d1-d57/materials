...
# ЯДРО БЛОКОВ — единственный дом словаря, разметки, парсера и модели блока для ОБОИХ
# движков ленты (заход `yadro-blokov`, 2026-08-13). До него этот файл держал свой
# словарь у себя (`_BOLD_STMT`/`_STMT_CLS`/`_STMT_NUM`/`_PROOF_HEAD`) и не был связан
# с `sborka/bloki.py` ничем — два дома одного знания расходились молча. Здесь остаётся
...
# Словарь жанров (`bloki.ZHANRY_VREZKI`), форма номера (`bloki.STMT_NUM`) и разметка зачина
# (`bloki.BOLD_STMT_RE`, `bloki.PROOF_HEAD_RE`) ЖИВУТ В ЯДРЕ и адресуются отсюда по полному
# имени — намеренно, без локальных алиасов: строка-алиас вида «прежнее имя, знак равенства,
# ссылка в ядро» выглядела бы вторым домом при любом грепе, а именно на греп опирается
# проверка «домов списка ровно один» (эта же строка комментария и поймалась на ней первой).
...
# ОТОБРАЖЕНИЕ ЖАНРА НА ВИД — собственность движка, в ядро не едет: класс и якорный ключ
# суть решения о картинке, а не о словаре. Таблица покрывает только исключения, общий
# случай («stmt thm», "t") стоит умолчанием в `_typed_block`.
VID_ZHANRA = {"Определение": ("stmt defn", "d"), "Пример": ("stmt task", "e"),
              "Задача": ("stmt task", "e"), "Замечание": ("stmt rem", "n")}

# 🔴 То, что КРАСНЕЕТ при расхождении домов (`KONSTITUCIYA §11`). Греп по имени переменной
# ловит второй дом только пока он назван прежним именем; переименуй его — и греп зелен, а
# словари разъехались. Здесь расхождение ловится по существу: жанр, которого ядро не знает,
# роняет импорт движка с внятным текстом, а не печатает молча врезку умолчательного вида.
_chuzhie = [z for z in VID_ZHANRA if z not in bloki.ZHANRY_VREZKI]
if _chuzhie:
    raise SystemExit(
        "build_doc: жанры %s есть в таблице вида, но не в словаре ядра "
        "(sborka/bloki.py: ZHANRY_VREZKI) — дом словаря обязан быть один" % ", ".join(_chuzhie))
...
def _body_html(text):
    ...
    СПИСОК ВНУТРИ ВРЕЗКИ (2026-08-05). Раньше строка `- пункт` в теле врезки попадала в общий
    абзац и печаталась дефисом посреди прозы, а вынести список пустой строкой было нельзя —
    пустая строка кончает блок, и список выпадал ИЗ врезки наружу. То есть определение, которое
    по смыслу есть список (данные, аксиомы), оформить было нечем; в корпусе это цена «пять блоков
    разрезаны, скелет доказательной структуры оборван». Теперь строки на `- ` внутри тела
    собираются в `<ul>`; продолжение пункта со следующей строки прирастает к тому же пункту."""
    out, buf, items = [], [], []
    ...
    def flush_list():
        if items:
            out.append("<ul>%s</ul>"
                       % "".join("<li>%s</li>" % render_inline(it) for it in items))
            items.clear()
    ...
    for raw_ln in text.split("\n"):
        ...
        if ln.startswith("- "):
            ...
            continue
        # перенос пункта — любая следующая строка, кроме жирного зачина абзаца и строки-формулы:
        # эти два закрывают список (иначе внутри врезки список нечем кончить — пустой строки там
        # быть не может, она кончает саму врезку).
        if items and not ln.startswith("**") and not (
                ln.startswith("$$") and ln.endswith("$$") and len(ln) > 4):
            items[-1] = items[-1] + " " + ln
            continue
        flush_list()
...
def _typed_block(joined, sid, proof_html=""):
    ...
    u = bloki.razobrat_zachin(joined)
    if u:
        # Разбор — ядро; всё ниже — ВИД, и он остаётся здесь. `imya` приходит из ядра уже
        # со срезанным «Статус: …» (Р-владелец 2026-08-04: параметр для автора, не для экрана;
        # в модели ядра он сохранён отдельным полем `u.status`, печатать его по-прежнему некому).
        kw, num, name, body = u.zhanr, u.nomer, u.imya, u.telo
        ...
        # номера может не быть вовсе (2026-08-05): тогда метка идёт без него, а якоря нет —
        # ссылаться «по утверждению …» не на что, и выдумывать номер за автора движок не станет.
        # 🔴 БУКВЕННЫЙ номер печатается ВСЕГДА, без флага `nomera: da`. Умолчание `SHOW_NUM=False`
        # заведено 2026-07-28 против ДВУХ ПЕРЕМЕШАННЫХ ЦИФРОВЫХ РЯДОВ («201, 202, 2, 203, 204,
        # 6, 205») — у буквенно-линейной нумерации такого столкновения нет по устройству: буква
        # и есть имя линии. Найдено верификатором 2026-08-05: без этой оговорки правка про любую
        # нумерацию ОТНИМАЛА видимую информацию — `**Утверждение A1.**` печаталось как
        # «Утверждение.», хотя тело документа ссылается на «утверждение A3».
        pokazat_nomer = bool(num) and (SHOW_NUM or not num.isdigit())
        label = ("%s %s%s." % (kw, num, name)) if pokazat_nomer else ("%s%s." % (kw, name))
        cls, key = VID_ZHANRA.get(kw, ("stmt thm", "t"))
        anchor = (' id="%s-%s-%s"' % (sid, key, num)) if num else ""
    ...
    if m:
        ...
        head = label.lstrip()
        if head.startswith(bloki.ZACHINY_VYVODA):
...
# 2026-08-05: номер в ссылке — той же формы, что в метке врезки (`bloki.STMT_NUM`), иначе «по
# утверждению A1» не находит свой якорь. Ссылка ставится только если якорь реально есть.
_REF_RE = re.compile(
    r"\b(утвержден|теорем|лемм|предложени|определени|пример|задач|замечани)(\w*)(\s+)("
    + bloki.STMT_NUM + r")(?![\wА-Яа-яЁё])")
...
def linkify_refs(html, sid):
    ...
    have = set(re.findall(r'id="(%s-[tden]-%s)"' % (re.escape(sid), bloki.STMT_NUM), html))
    if not have:
        return html
...
def render_stream(body, sid="s0"):
    ...
    while i < len(blocks):
        ...
        # 6. список. ПУНКТ МОЖЕТ БЫТЬ В НЕСКОЛЬКО СТРОК (2026-08-05): продолжение пишется С
        #    ОТСТУПОМ и прирастает к своему пункту. Раньше условие было `all(… startswith("- "))`,
        #    и один перенесённый пункт ронял ВЕСЬ блок в ветку 7 — список печатался абзацем
        #    `<p>- пункт - пункт</p>`. Цена: на первой сборке ленты Л2 так склеились 7 списков из
        #    8 (`teorkat-vvedenie/UROKI-FABRIKE-lenta.md` F5); чинили тогда источник, не движок.
        #    Блок, где все строки на `- `, разбирается ровно как прежде.
        #    Перенос ловится и с отступом, и без него: в живом корпусе встретились оба (в
        #    `02-liniya-B.md` продолжение пункта идёт вровень и несёт `$$…$$`).
        if first.startswith("- "):
            items = []
            for l in lines:
                if l.lstrip().startswith("- "):
                    items.append(l.lstrip()[2:].strip())
                else:
        ...
        raw = "\n".join(lines)
        proof_html = ""
        if (bloki.BOLD_STMT_RE.match(raw) and i < len(blocks)
                and bloki.PROOF_HEAD_RE.match(blocks[i].strip())):
...
/* .s-node-a — акцентный узел. Словарь примитивов (_illustracii/DISCIPLINA.md, строка «узел») обещал его
