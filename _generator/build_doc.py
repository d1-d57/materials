...
# 2026-08-05: номер в ссылке — той же формы, что в метке врезки (`_STMT_NUM`), иначе «по
...
def linkify_refs(html, sid):
    ...
    have = set(re.findall(r'id="(%s-[tden]-%s)"' % (re.escape(sid), _STMT_NUM), html))
    if not have:
        return html
