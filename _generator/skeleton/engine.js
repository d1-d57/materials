...
function paintBleed(slide) {
  ...
  /* 🔴 ПОУГЛОВОЙ ГАСИТЕЛЬ СНЕСЁН (заход sloi-i-obrez, Э3). Здесь стоял
     `cornerBlocked(x, y)`: `elementFromPoint` в пикселе угла плюс цепочка предков,
     и по «закрыт ли угол» решалось, рисовать ли узор в поле. Он отвечал НЕ НА ТОТ
     вопрос. Требование владельца было «рисуй поверх зелёного», а механизм отвечал
     «закрыт ли угол» — поэтому его чинили дважды (`polya-i-uzor`, потом
     `kadr-uzor-i-vmeshchenie`), оба раза верно по своей логике, и оба раза узор
     оставался разрезанным границей зелёной зоны. Настоящий ответ — слой:
     `.slide::after` в `uzor.css` рисует узор ВЫШЕ фоновых зон, и внутри кадра
     гасить теперь нечего и незачем.

     Осталась ровно одна законная причина погасить узор СНАРУЖИ — «внутри кадра его
     нет вовсе». Иначе получается полурисуемый узор (в поле есть, в кадре нет),
     который владелец назвал худшим из трёх исходов: «если оставлять, то он должен
     полностью рисоваться». Таких случаев два, и оба видны БЕЗ ощупывания углов:
       · служебный слайд со сплошным фоном — `tipy._sploshnoj_fon` пишет ему
         `::after{content:none}`, то есть слоя узора у него физически нет;
       · полнокадровый растр — картинка закрывает узор во всём кадре (её же
         поднимает над слоем `z-index:2`).
     Признак читается с самого слоя, а не выводится заново: одна правда, объявленная
     в CSS, и один её читатель здесь. */
  const bezUzora = !!polnokadrovyj ||
        getComputedStyle(slide, '::after').content === 'none';
  bleedUzor.style.display = bezUzora ? 'none' : '';
}
