# Канал исполнителя — otrisovka-vidov (один заход до конца)
...
> **Модель: Haiku 4.5** — a small rendering fix in an existing generator with a machine criterion; mechanical.
...
```
Модель: Haiku 4.5 — <одна фраза почему; см. шапку захода>.

Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/materials.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 41
...
🔴 ДВЕРЬ ГЕЙТА СБОРКИ ЗАПОЛНЕННОГО ФАЙЛА ОТКРЫТА КЛАПАНОМ: в арке `_studio/zhurnal/2026-08-24_obzor-funkciya-putey` лежат файлы-заходы со штампом «ЖДЁТ», красные на `check_sborki.py` — `kod_graf-korpusa.md`, `kod_konsolidaciya-korpusa.md`, `kod_pasporta-korpusa.md` — следующий заход в такую арку обычно не собирается, но АНАЛИТИК открыл клапан, причина дословно: «the red briefs are already ACCEPTED positions of this wave (verdicts in the mandate); their reds are sandbox paths and criteria on files the positions created». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.

КОНТЕКСТ. The course plans of «Пути и волны» are generated by `kurs-puti-i-volny/tools/plany.py` from the one items file `kurs-puti-i-volny/plan/src/punkty.md`. The items and level summaries are written; the generator drops part of them. ЦЕЛЬ: the views show what the file holds.
Приёмка — по ОТЧЁТУ, без построчной сверки. If you stop early: commit what renders.
...
1. The item is an atom living once in punkty.md; all plan views are generated by plany.py and never hand-written
2. Every plan opens with ONE sentence saying how it generalises the finer plan
...
- **ЗОНА (можно менять):** `kurs-puti-i-volny/tools/plany.py` `kurs-puti-i-volny/plan/src` `_studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone kurs-puti-i-volny/tools/plany.py kurs-puti-i-volny/plan/src _studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md
```
...
- Точка отката: `git add kurs-puti-i-volny/tools/plany.py kurs-puti-i-volny/plan/src _studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
### Three rendering defects of `tools/plany.py` — fix exactly these, no content edits
Read ONLY `kurs-puti-i-volny/tools/plany.py` and the preface plus the level blocks of `kurs-puti-i-volny/plan/src/punkty.md`. Do not edit any item field or level text — they are written by someone else and are final.
1. **Level blocks are not rendered.** `punkty.md` has level blocks (`god`, `chast-do-analiza`, `chetvert-1`, …) with fields `obobshchenie` and `svod`; the parser keeps only the keys in `ITEM_FIELDS`, so every view says the summary is not written. Parse them and make EVERY view open with its level's `obobshchenie` (one sentence, bold) and then `svod`, right under the title: `god.md` ← level `god`; `chast-1-do-analiza.md` ← `chast-do-analiza`; `chetvert-N.md` ← `chetvert-N`; `lekciya-1.md` ← the `obobshchenie` of `chetvert-1` is NOT used — the lecture opens with the item's `vopros`. An empty level field renders as «ещё не написано», as now.
2. **Names are missing in headings.** The lecture view is titled «Лекция 1 — p-01» with a section «### p-01»; the quarter and year views likewise. Every item heading must read `p-NN · <imya>` (the id stays for the reader; the anchor comments `<!--id: …-->` and the `[[…]]` links must stay exactly as they are now).
3. **The preface of `punkty.md` says `raskadrovka` and the level fields are empty** — that is now false. Correct only those sentences of the preface (the preface is form, not content).

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ)** — each clause one command, return code first:
1. **Live run on the real items file:** `python3 kurs-puti-i-volny/tools/plany.py; echo $?` → `0`, then `python3 kurs-puti-i-volny/tools/plany.py --proverit; echo $?` → `0`.
2. `grep -c 'ещё не написано' kurs-puti-i-volny/plan/src/chetvert-1.md` → `0`, and the same for `god.md` and `chast-1-do-analiza.md` (their level blocks are filled).
3. `grep -c 'p-01 · ' kurs-puti-i-volny/plan/src/lekciya-1.md` ≥ 1.
4. `python3 kurs-puti-i-volny/tools/graf.py | grep '^\[c\]'` → orphans 0, and `cd kurs-puti-i-volny && python3 tools/indeks.py; echo $?` → `0` with no dangling id.
Commit and push by path: `GIT_ZONA_REPO="$PWD" python3 ../disciplina/_generator/tools/git_zona.py commit --zone kurs-puti-i-volny/tools/plany.py --zone kurs-puti-i-volny/plan/src --zone _studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md -m "<what>" --push`. Other people's uncommitted files lie in this folder — never commit them.

**Coverage line in the report:** «defects fixed N of 3 · views regenerated 7 of 7».
...
Верификатор не нужен: the criterion is fully machine-checked by grep on the generated views.
...
```
git --no-optional-locks add -- kurs-puti-i-volny/tools/plany.py kurs-puti-i-volny/plan/src _studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- kurs-puti-i-volny/tools/plany.py kurs-puti-i-volny/plan/src _studio/zhurnal/2026-08-24_obzor-funkciya-putey/kod_otrisovka-vidov.md   # отсекает всё чужое
