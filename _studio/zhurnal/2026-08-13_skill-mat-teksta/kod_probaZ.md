# Канал исполнителя — probaZ (один заход до конца)
...
Ты исполнитель в репозитории /sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi.

Твой единственный вход — файл-заход:
/sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi/_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/probaZ`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/korni.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaZ.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
