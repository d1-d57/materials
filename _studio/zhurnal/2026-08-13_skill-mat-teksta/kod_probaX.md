# Канал исполнителя — probaX (один заход до конца)
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-probaX.jsonl
```
...
- **МЕСТО РАБОТЫ:** ветка `zahod/probaX` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/probaX`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/korni.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaX.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
