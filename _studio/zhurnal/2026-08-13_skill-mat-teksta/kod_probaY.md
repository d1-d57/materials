# Канал исполнителя — probaY (один заход до конца)
...
```
cd /sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi && claude -p --verbose --output-format stream-json --model arn:aws:bedrock:us-east-1:811345154057:application-inference-profile/gn8yl4ks1php --dangerously-skip-permissions 'Твой заход — файл /sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi/_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md. Прочитай ТОЛЬКО его и то, что он называет; остальной проект не изучай. План/вопросы/отчёт пиши в этот же файл внизу (## ПЛАН / ## ВОПРОСЫ / ## ОТЧЁТ). Ничего сверх задачи не трогай — «ничего сверх задачи» относится к СОДЕРЖАНИЮ работы; git-контур §0.1 — законное исключение, он про состояние репозитория и исполняется целиком.' < /dev/null 2>&1 | tee /tmp/zahod-probaY.jsonl | python3 -u -c 'import sys,json
for l in sys.stdin:
 try:
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-probaY.jsonl
```
...
- **МЕСТО РАБОТЫ:** ветка `zahod/probaY` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
🔴 <НЕ ЗАПОЛНЕНО — сторож невлитого не отработал при сборке (fatal: not a git repository: /Users/ivanyakovlev/Documents/GitHub/materials/.git/worktrees/rychagi); состояние чужих веток НЕ проверено — снять руками `git branch --no-merged` и вписать сюда результат>
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/probaY`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/korni.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_probaY.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
- **Г2. Второй репозиторий.** **неприменимо, и это проверено при сборке, а не предположено:** все пути зоны лежат внутри репозитория `rychagi` (тот же критерий, что у С2 `check_sborki.py`). Зона расширилась за его пределы по ходу — пункт снова применим; команда та же, что в применимом случае: `cd ../<репозиторий> && git --no-optional-locks status --porcelain` → пусто. *Команда названа и здесь нарочно (находка верификатора): пункт, который объявлен неприменимым и не говорит, ЧТО делать, когда станет применим, исполнить в этот момент нечем.*
- **Г3. Невлитых веток не прибавилось.** 🔴 <НЕ ЗАПОЛНЕНО — база сравнения веток> — при сборке снять не удалось (fatal: not a git repository: /Users/ivanyakovlev/Documents/GitHub/materials/.git/worktrees/rychagi). Подставь ветку арки сам и скажи об этом в отчёте.
