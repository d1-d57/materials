# Канал исполнителя — prZ (один заход до конца)
...
Ты исполнитель в репозитории /sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi-3.

Твой единственный вход — файл-заход:
/sessions/festive-sweet-clarke/mnt/GitHub/materials-wt/rychagi-3/_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md
...
🔴 <НЕ ЗАПОЛНЕНО — сторож невлитого не отработал при сборке (fatal: not a git repository: /Users/ivanyakovlev/Documents/GitHub/materials/.git/worktrees/rychagi-3); состояние чужих веток НЕ проверено — снять руками `git branch --no-merged` и вписать сюда результат>
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/prZ`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/korni.py _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/korni.py` `_studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/korni.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_skill-mat-teksta/kod_prZ.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
- **Г2. Второй репозиторий.** **неприменимо, и это проверено при сборке, а не предположено:** все пути зоны лежат внутри репозитория `rychagi-3` (тот же критерий, что у С2 `check_sborki.py`). Зона расширилась за его пределы по ходу — пункт снова применим; команда та же, что в применимом случае: `cd ../<репозиторий> && git --no-optional-locks status --porcelain` → пусто. *Команда названа и здесь нарочно (находка верификатора): пункт, который объявлен неприменимым и не говорит, ЧТО делать, когда станет применим, исполнить в этот момент нечем.*
- **Г3. Невлитых веток не прибавилось.** 🔴 <НЕ ЗАПОЛНЕНО — база сравнения веток> — при сборке снять не удалось (fatal: not a git repository: /Users/ivanyakovlev/Documents/GitHub/materials/.git/worktrees/rychagi-3). Подставь ветку арки сам и скажи об этом в отчёте.
