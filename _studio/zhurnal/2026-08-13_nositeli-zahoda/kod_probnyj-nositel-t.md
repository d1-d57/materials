# Канал исполнителя — probnyj-nositel-t (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/materials && claude -p --verbose --output-format stream-json --model arn:aws:bedrock:us-east-1:811345154057:application-inference-profile/gn8yl4ks1php --dangerously-skip-permissions 'Твой заход — файл /Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md. Прочитай ТОЛЬКО его и то, что он называет; остальной проект не изучай. План/вопросы/отчёт пиши в этот же файл внизу (## ПЛАН / ## ВОПРОСЫ / ## ОТЧЁТ). Ничего сверх задачи не трогай.' < /dev/null 2>&1 | tee /tmp/zahod-probnyj-nositel-t.jsonl | python3 -u -c 'import sys,json
for l in sys.stdin:
 try:
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-probnyj-nositel-t.jsonl
```
...
1. проба секции-носителя, канал terminal
2. второе подтверждение для Ш2.1

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/probnyj-nositel-t` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `_generator/tools/proba-nositel-zona-t.py` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- пробная зона: `_generator/tools/proba-nositel-zona-t.py`, `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md` (дозаполнено вручную для критерия готовности захода `nositel`, п.3 — проба одноразовая, коммита не будет).
...
- гасить нечего: проба одноразовая, ветку `zahod/probnyj-nositel-t` не заводили (дозаполнено вручную для критерия п.3).

- Проверь ветку: `git branch --show-current` — обязано быть `zahod/probnyj-nositel-t`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/proba-nositel-zona-t.py` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
## 2. ЗАДАЧА
Проба секции-носителя для критерия готовности захода `nositel` (дозаполнено вручную). **КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):** 1. `grep -c '^## СТАРТОВОЕ СООБЩЕНИЕ ВЛАДЕЛЬЦУ' _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md` → 1. 2. 🔴 Живой прогон на реальном файле репозитория, не только фикстура.
...
```
git --no-optional-locks add -- _generator/tools/proba-nositel-zona-t.py _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/proba-nositel-zona-t.py _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/proba-nositel-zona-t.py` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/proba-nositel-zona-t.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel-t.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
