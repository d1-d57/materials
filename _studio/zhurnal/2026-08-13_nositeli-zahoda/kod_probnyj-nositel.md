# Канал исполнителя — probnyj-nositel (один заход до конца)
...
/Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md
...
- пробная зона: `_generator/tools/proba-nositel-zona.py`, `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md` (дозаполнено вручную для критерия готовности захода `nositel`, п.3 — проба одноразовая, коммита не будет).
...
- гасить нечего: проба одноразовая, ветку `zahod/probnyj-nositel` не заводили (дозаполнено вручную для критерия п.3).

- Проверь ветку: `git branch --show-current` — обязано быть `zahod/probnyj-nositel`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/proba-nositel-zona.py` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
## 2. ЗАДАЧА
Проба секции-носителя для критерия готовности захода `nositel` (дозаполнено вручную). **КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):** 1. `grep -c '^## СТАРТОВОЕ СООБЩЕНИЕ ВЛАДЕЛЬЦУ' _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md` → 1. 2. 🔴 Живой прогон на реальном файле репозитория, не только фикстура.
...
```
git --no-optional-locks add -- _generator/tools/proba-nositel-zona.py _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/proba-nositel-zona.py _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/proba-nositel-zona.py` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/proba-nositel-zona.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_probnyj-nositel.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
