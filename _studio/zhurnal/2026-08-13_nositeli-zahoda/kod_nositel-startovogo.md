# Канал исполнителя — nositel-startovogo (один заход до конца)
...
- **МЕСТО РАБОТЫ:** ветка `zahod/nositel-startovogo` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `_generator/tools/bootstrap_zahod.py` `_generator/tools/check_zahod.py` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/nositel-startovogo`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_generator/tools/bootstrap_zahod.py` `_generator/tools/check_zahod.py` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/bootstrap_zahod.py _generator/tools/check_zahod.py _generator/tools/fixtures/zahod/PROGNAT.sh _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/bootstrap_zahod.py _generator/tools/check_zahod.py _generator/tools/fixtures/zahod/PROGNAT.sh _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/bootstrap_zahod.py` `_generator/tools/check_zahod.py` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/bootstrap_zahod.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/check_zahod.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/fixtures/zahod/PROGNAT.sh` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-13_nositeli-zahoda/kod_nositel-startovogo.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
