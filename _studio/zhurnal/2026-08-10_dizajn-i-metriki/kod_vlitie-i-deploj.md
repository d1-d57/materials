# Канал исполнителя — vlitie-i-deploj (один заход до конца)
...
/Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md
...
- **МЕСТО РАБОТЫ:** ветка `zahod/vlitie-i-deploj` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `_studio/zhurnal/_INFRA-git/,` `_studio/docs/KARTA.md,` `sayt/` `_studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/vlitie-i-deploj`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add `_studio/zhurnal/_INFRA-git/,` `_studio/docs/KARTA.md,` `sayt/` `_studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _studio/zhurnal/_INFRA-git/, _studio/docs/KARTA.md, sayt/ _studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _studio/zhurnal/_INFRA-git/, _studio/docs/KARTA.md, sayt/ _studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_studio/zhurnal/_INFRA-git/,` `_studio/docs/KARTA.md,` `sayt/` `_studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/_INFRA-git/,` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/docs/KARTA.md,` → ✅; `python3 _generator/tools/git_zona.py check --zone sayt/` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/2026-08-10_dizajn-i-metriki/kod_vlitie-i-deploj.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
