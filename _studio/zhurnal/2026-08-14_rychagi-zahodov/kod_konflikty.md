# Канал исполнителя — konflikty (один заход до конца)
...
/Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-14_rychagi-zahodov/kod_konflikty.md
...
2. конфликты разрешаются ПО СУЩЕСТВУ: за каждым стоит живой дефект или устаревшая сторона, а не расхождение текстов; выбрать сторону вслепую значит вернуть дефект или молча отменить чужую работу
3. по disciplina сторона уже установлена фактом предыдущим заходом: основная права, ветка от 08.08 устарела — проверено gejt_kartochki.py --help | grep -c faza = 3
4. модель Opus: каждое решение — суждение о чужой подсистеме, механики здесь нет
...
- **ЗОНА (можно менять):** `_generator/sborka/` `_generator/tools/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 _generator/tools/git_zona.py merge zahod/git-kontur --zone _generator/sborka/ --zone _generator/tools/ --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/
```
...
- Точка отката: `git add `_generator/sborka/` `_generator/tools/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/sborka/ _generator/tools/ /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/sborka/ _generator/tools/ /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/sborka/` `_generator/tools/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/sborka/` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/` → ✅; `python3 _generator/tools/git_zona.py check --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
