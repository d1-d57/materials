# Канал исполнителя — dolgi-zero (один заход до конца)
...
Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/materials-wt/dolgi-zero.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/materials/_studio/zhurnal/2026-08-14_rychagi-zahodov/kod_dolgi-zero.md
...
Первым ходом: python3 _generator/tools/git_zona.py worktree add dolgi-zero --branch zahod/dolgi-zero, затем чтение захода, затем секция
...
3. замер приёмки 14.08: гейта на накопление НЕТ ВОВСЕ — priyomka.py не проверяет ни невлитое (grep no-merged = 0), ни невывезенное (0), ни грязное (0); значит долг накопится снова и никто не заметит
...
- **МЕСТО РАБОТЫ:** **рабочая папка `../materials-wt/dolgi-zero`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/dolgi-zero`. 🔴 **Заводишь её САМ, первым ходом, из корня репозитория:**
  ```
  python3 _generator/tools/git_zona.py worktree add dolgi-zero --branch zahod/dolgi-zero
  cd ../materials-wt/dolgi-zero
  ```
...
- **ЗОНА (можно менять):** `_generator/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/doma/`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 _generator/tools/git_zona.py merge zahod/konflikty --zone _generator/ --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/ --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/doma/
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/dolgi-zero`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add `_generator/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/doma/`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/ /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/ /Users/ivanyakovlev/Documents/GitHub/disciplina/doma/                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/ /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/ /Users/ivanyakovlev/Documents/GitHub/disciplina/doma/   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/` `/Users/ivanyakovlev/Documents/GitHub/disciplina/doma/`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/` → ✅; `python3 _generator/tools/git_zona.py check --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/skills/slajdy/` → ✅; `python3 _generator/tools/git_zona.py check --zone /Users/ivanyakovlev/Documents/GitHub/disciplina/doma/` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
