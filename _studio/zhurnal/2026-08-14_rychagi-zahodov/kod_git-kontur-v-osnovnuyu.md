# Канал исполнителя — git-kontur-v-osnovnuyu (один заход до конца)
...
1. вариант А, решение владельца 14.08: субагент с ОТДЕЛЬНЫМИ правами делает git-работу, вливает в основную ветку — и только потом заход заводит свою ветку от свежей основной и работает как раньше
...
- **ЗОНА (можно менять):** `_generator/tools/git_zona.py` `_generator/tools/bootstrap_zahod.py` `_generator/tools/fixtures/git_zona/PROGNAT.sh` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/_TEMPLATE-zahod.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 _generator/tools/git_zona.py merge zahod/rychagi-3 --zone _generator/tools/git_zona.py --zone _generator/tools/bootstrap_zahod.py --zone _generator/tools/fixtures/git_zona/PROGNAT.sh --zone _generator/tools/fixtures/zahod/PROGNAT.sh --zone _studio/zhurnal/_TEMPLATE-zahod.md
```
...
- Точка отката: `git add `_generator/tools/git_zona.py` `_generator/tools/bootstrap_zahod.py` `_generator/tools/fixtures/git_zona/PROGNAT.sh` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/_TEMPLATE-zahod.md`` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _generator/tools/git_zona.py _generator/tools/bootstrap_zahod.py _generator/tools/fixtures/git_zona/PROGNAT.sh _generator/tools/fixtures/zahod/PROGNAT.sh _studio/zhurnal/_TEMPLATE-zahod.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _generator/tools/git_zona.py _generator/tools/bootstrap_zahod.py _generator/tools/fixtures/git_zona/PROGNAT.sh _generator/tools/fixtures/zahod/PROGNAT.sh _studio/zhurnal/_TEMPLATE-zahod.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `_generator/tools/git_zona.py` `_generator/tools/bootstrap_zahod.py` `_generator/tools/fixtures/git_zona/PROGNAT.sh` `_generator/tools/fixtures/zahod/PROGNAT.sh` `_studio/zhurnal/_TEMPLATE-zahod.md`

- **Г1. Зона доехала в git.** `python3 _generator/tools/git_zona.py check --zone _generator/tools/git_zona.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/bootstrap_zahod.py` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/fixtures/git_zona/PROGNAT.sh` → ✅; `python3 _generator/tools/git_zona.py check --zone _generator/tools/fixtures/zahod/PROGNAT.sh` → ✅; `python3 _generator/tools/git_zona.py check --zone _studio/zhurnal/_TEMPLATE-zahod.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
