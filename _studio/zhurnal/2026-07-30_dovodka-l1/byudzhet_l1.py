...
· гейт 3 (полнота ТЗ) · гейт 4 (бюджет по 15 строкам) · гейт 5 (сохранность блоков
относительно HEAD). Ничего не пишет на диск. Код возврата: 0 — все гейты зелёные.
...
# Намерение исполнителя (заход kod_geometria.md). Слайд 1 — обложка: служебный
# слой, геометрии не имеет (07-verstka/DOK.md §13; porodit.py:481 «в slide_order
# не входит»; gejt_lenty.py:123 требует пометку «кроме обложки»).
HOTEL = {2: "илл-полосой-снизу", 3: "лестница-во-всю-ширину",
         4: "илл-полосой-снизу", 5: "илл-полосой-снизу", 6: "рейка-справа",
         7: "рейка-справа", 8: "рейка-справа", 9: "илл-полосой-снизу",
         10: "рейка-справа", 11: "рейка-справа", 12: "доска-пустая",
         13: "рейка-справа", 14: "доска-пустая", 15: "рейка-справа"}
# Слайды, где число дефицита НЕ действующее: решение владельца не принято.
NEDEJSTVUYUSHCHIE = {2: "задача не придумана", 14: "содержание не названо"}
...
def main():
    ...
    for s in ss:
        ...
        rash += 0 if ok else 1
        print("%-3d %-40s %-23s %-23s %s"
              % (s["n"], s["title"][:40], HOTEL[s["n"]], got, "✅" if ok else "❌ РАСХОЖДЕНИЕ"))
    ...
    for s in ss:
        if s["n"] in HOTEL and P.archetype_lenty(s) != P.archetype(s):
    ...
    for s in ss:
        ...
        cel = round(vm * ZAPAS)
        d = cel - gl
        met = "  🕳 %s" % NEDEJSTVUYUSHCHIE[s["n"]] if s["n"] in NEDEJSTVUYUSHCHIE else ""
        if d > 0:
            sum_def += d
            if s["n"] not in NEDEJSTVUYUSHCHIE:
                sum_dejstv += d
        else:
            sum_prof += -d
