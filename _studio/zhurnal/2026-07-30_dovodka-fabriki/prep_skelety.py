...
def skelet_vladelca():
    ...
    for i, z in enumerate(zs, 1):
        ...
        rows.append("\t".join([
            ...
            rez(z["_zagolovok"], 300),
            rez(z.get("АДРЕС", ""), 160),
            rez(z.get("КАСАЕТСЯ", ""), 30),
            rez(z.get("КЛАСС", ""), 20),
            # Цитата нужна для СУЖДЕНИЯ: с ней разметчик не открывает корпус.
            # ЦЕНА обрезки, оплачено заходом H1 01.08: при отсечке заголовка на
            # 90 знаках исполнитель СОЗНАТЕЛЬНО читал все 28 корпусов целиком —
            # «судить механизм по обрубку значит судить по половине
            # предложения», — и протащил через контекст 523 КБ вместо 216.
            # Замер: снятие обрезки стоит 4 КБ на весь скелет (212 → 216 КБ).
            # Обрезка экономила байты и покупала на них перечитывание корпуса.
            rez(z.get("ЦИТАТА", ""), 500),
        ]))
