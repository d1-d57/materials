# Канал исполнителя — proba-komandy (один заход до конца)
...
- **ЗОНА (можно менять):** _studio/zhurnal/_INFRA-git _studio/zhurnal/_INFRA-git/kod_proba-komandy.md. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Точка отката: `git add _studio/zhurnal/_INFRA-git _studio/zhurnal/_INFRA-git/kod_proba-komandy.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
```
git --no-optional-locks add -- _studio/zhurnal/_INFRA-git _studio/zhurnal/_INFRA-git/kod_proba-komandy.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- _studio/zhurnal/_INFRA-git _studio/zhurnal/_INFRA-git/kod_proba-komandy.md   # отсекает всё чужое
