...
> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: якорь всех доказательств блока — два этажа и подъём конструкции между ними. Дальше вернётся та же геометрия с другими обитателями.
...
Сильная форма: возьмём вложение $s\colon C_2\to S_3$ подгруппы транспозиции и знак $\mathrm{sgn}\colon S_3\to C_2$; транспозиция нечётна, поэтому $\mathrm{sgn}\circ s=1_{C_2}$

Функтор сохранил бы эту пару, но $Z(C_2)=C_2$, $Z(S_3)=1$, а $C_2\to 1\to C_2$ тождественным не бывает
...
## Раскраски без повторов

> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: пять точек и четыре цвета.

> поле:mn **SPLIT.** Слайд отделён от предыдущего этим заходом: в конспекте обе сцены и падение имён стоят одной секцией на три сцены, и в одном разделе ленты это давало больше 250 слов при потолке 75. Причина и цена — в отчёте захода.

**Утверждение.** Сопоставление $X\mapsto\{\text{инъекции }X\hookrightarrow\{1,\dots,k\}\}$ не продолжается до функтора $\mathbf{Set}\to\mathbf{Set}$ ни ковариантно, ни контравариантно

Возьмём $\lvert A\rvert=k$, $\lvert B\rvert=k+1$: вложение $s\colon A\to B$ и $r\colon B\to A$, тождественное на образе, дают $r\circ s=1_A$

Пишем $FX$ для этих инъекций: $\lvert FA\rvert=k!$, а $FB=\varnothing$
...
## Ретракция, сечение, ретракт

> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: треугольник, в котором путь туда и обратно даёт тождество.

> поле:mn **SPLIT.** Третий раздел той же секции конспекта: имена и общий факт падают на два слайда, потому что четыре формулировки в одном не держатся.
...
**Определение.** Морфизм $f\colon A\to B$ называется изоморфизмом, если существует $g\colon B\to A$ с $g\circ f=1_A$ и $f\circ g=1_B$; объекты $A$ и $B$ изоморфны, $A\cong B$

**Определение.** Пусть морфизмы $r\colon B\to A$ и $s\colon A\to B$ таковы, что
...
## Что переживает функтор

> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: тот же якорь в общем виде.

> поле:mn **SPLIT.** Вторая половина третьего раздела: имена уже упали, здесь стоят три утверждения о том, что с ними делает функтор.

**Утверждение.** Если $r\circ s=1_A$, то $Fr\circ Fs=1_{FA}$ для любого функтора $F$: значит $FA$ есть ретракт $FB$

Доказательство в одну строку: $Fr\circ Fs=F(r\circ s)=F(1_A)=1_{FA}$

**Утверждение.** Если $f$ изоморфизм, то $Ff$ изоморфизм, причём $(Ff)^{-1}=F(f^{-1})$

**Утверждение.** Если $A$ есть ретракт $B$, а между $FA$ и $FB$ нет пары морфизмов с тождественной композицией, то конструкция $A\mapsto FA$ функтором не продолжается
...
## Конструкция, уменьшающая размер

> поле:mn **Раскладка.** Мало текста, крупная горизонтальная иллюстрация снизу: согласованность с отображениями целиком. Пока зал думает, на слайде нет ничего, кроме условия.

**Задача Б1.** Существует ли конструкция, сопоставляющая конечному множеству множество строго меньшего размера согласованно с отображениями?
...
> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: окружность и диск, под ними их фундаментальные группы.

> поле:mn **SPLIT.** Секция конспекта слита из двух слайдов и здесь снова разведена: условие задачи стоит отдельно, потому что зал над ним работает, а топологическая сцена несёт две формулировки и в один экран с задачей не входит.

**Утверждение.** Окружность $S^1$ не является ретрактом диска $D^2$ в категории топологических пространств и непрерывных отображений

Фундаментальная группа $\pi_1$ есть функтор из пространств с отмеченной точкой в группы, и $\pi_1(S^1)\cong\mathbb Z$, $\pi_1(D^2)=0$

Пусть $r\circ i=1_{S^1}$ для вложения $i\colon S^1\to D^2$. Тогда тождественный морфизм группы $\mathbb Z$ разложился бы через нулевую группу: $\mathbb Z\to 0\to\mathbb Z$
...
> поле:mn **Раскладка.** Много текста, вертикальная полоса справа: луч внутри диска, ниже портрет.

> поле:mn **SPLIT.** Вторая формулировка топологической сцены: следствие про неподвижную точку вынесено отдельно, иначе на слайде стояли бы две теоремы с двумя доказательствами.

**Теорема (Брауэр).** Всякое непрерывное отображение $f\colon D^2\to D^2$ имеет неподвижную точку

Скомкать карту города и уронить внутрь того же города: найдётся точка карты ровно над местом, которое она изображает

Пусть $f(x)\ne x$ для всех $x$. Тогда луч из $f(x)$ через $x$ упирается в границу, и это задаёт непрерывное $r\colon D^2\to S^1$

Для $x\in S^1$ луч упирается в саму $x$, значит $r\circ i=1_{S^1}$, чего быть не может

🖼 Портрет Лёйтзена Брауэра {3}
