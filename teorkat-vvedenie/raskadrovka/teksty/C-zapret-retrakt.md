...
> поле:mn **Раскладка.** Много текста, вертикальная полоса справа, по картинке на сцену: два этажа центров (1), носитель против палитры (2–3), треугольник ретракта (4), тот же якорь в общем виде (5). Вопрос-заголовок стоит на сценах 1–2, пока идут два примера; дальше общую структуру держат заголовок слайда и якорный чертёж, а определение ретракта доживает до сцены 5, где слово «ретракт» работает в трёх утверждениях.

> поле:mn **Порядок владельца.** Сначала два примера, потом имена, потом общий факт: определения ретракции и ретракта приходят на сцене 4, после того как зал два раза увидел приём в работе.
...
{@1-1} Сильная форма: вложение транспозиции $s\colon C_2\to S_3$ и знак $\mathrm{sgn}\colon S_3\to C_2$ дают $\mathrm{sgn}\circ s=1_{C_2}$, потому что транспозиция нечётна. Функтор сохранил бы эту пару, но $Z(C_2)=C_2$, $Z(S_3)=1$, а $C_2\to 1\to C_2$ тождественным не бывает

{@2-3} **Утверждение.** Функтора $\mathbf{Set}\to\mathbf{Set}$, совпадающего на объектах с правилом $X\mapsto\{\text{инъекции }X\hookrightarrow\{1,\dots,k\}\}$, не существует — ни ковариантного, ни контравариантного

{@2-2} $\lvert A\rvert=k$, $\lvert B\rvert=k+1$: вложение $s\colon A\to B$ и $r\colon B\to A$, тождественное на образе, дают $r\circ s=1_A$

{@3-3} Пишем $FX$ для этих инъекций: $\lvert FA\rvert=k!$, а $FB=\varnothing$. Функтор дал бы $FA\to FB\to FA$ с тождественной композицией, а в пустое множество отображает только пустое
...
{@4-4} **Определение.** Морфизм $f\colon A\to B$ — изоморфизм, если существует $g\colon B\to A$ с $g\circ f=1_A$ и $f\circ g=1_B$; объекты $A$ и $B$ изоморфны, $A\cong B$

{@4-5} **Определение.** Если морфизмы $r\colon B\to A$ и $s\colon A\to B$ дают $r\circ s=1_A$, то $r$ — ретракция, $s$ — сечение, а $A$ — ретракт объекта $B$
...
{@5} **Утверждение.** Если $r\circ s=1_A$, то $Fr\circ Fs=1_{FA}$ для любого функтора $F$: значит, $FA$ ретракт $FB$

{@5} Доказательство в одну строку: $Fr\circ Fs=F(r\circ s)=F(1_A)=1_{FA}$

{@5} **Утверждение.** Если $f$ изоморфизм, то $Ff$ изоморфизм, причём $(Ff)^{-1}=F(f^{-1})$

{@5} **Утверждение.** Если $A$ ретракт $B$, а между $FA$ и $FB$ нет пары морфизмов с тождественной композицией, то конструкция $A\mapsto FA$ функтором не продолжается
...
> поле:mn **Раскладка.** Мало текста, крупная горизонтальная иллюстрация снизу: на сцене 1 согласованность с отображениями целиком, дальше окружность в диске (2) и луч внутри диска с портретом (3). Пока зал думает, на слайде нет ничего, кроме условия задачи.

{@1-1} **Задача Б1.** Существует ли конструкция, сопоставляющая конечному множеству множество строго меньшего размера согласованно с отображениями?

{@2} **Утверждение.** Окружность $S^1$ не ретракт диска $D^2$ в $\mathbf{Top}$

{@2-2} Фундаментальная группа $\pi_1$ — функтор из пространств с отмеченной точкой в $\mathbf{Grp}$; $\pi_1(S^1)\cong\mathbb Z$, $\pi_1(D^2)=0$

{@2-2} Пусть $r\circ i=1_{S^1}$ для вложения $i\colon S^1\to D^2$. Тогда тождественный морфизм $\mathbb Z$ разложился бы как $\mathbb Z\to 0\to\mathbb Z$, а такое тождественно лишь при $\mathbb Z=0$

{@3} **Теорема (Брауэр).** Всякое непрерывное отображение $f\colon D^2\to D^2$ имеет неподвижную точку

{@3} Скомкать карту города и уронить внутрь того же города: найдётся точка карты ровно над местом, которое она изображает

{@3} Пусть $f(x)\ne x$ всюду. Луч из $f(x)$ через $x$ упирается в границу, и это задаёт непрерывное $r\colon D^2\to S^1$. Для $x\in S^1$ луч упирается в саму $x$, значит $r\circ i=1_{S^1}$, чего быть не может
