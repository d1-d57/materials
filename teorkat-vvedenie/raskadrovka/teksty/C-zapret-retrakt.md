...
## Инъекции в палитру: функтора нет

> поле:mn **Раскладка.** Мало текста, полоса справа пустая — иллюстраций у слайда нет.

> поле:mn **НОВЫЙ слайд, правка владельца 29.07** (`PRAVKI-final.md` §12.2): *«пропадает слайд про инъекции: слайд про центр есть, а предыдущего перед ним нету, хотя у нас было продумано то же самое про инъекции»*. Стоит ПЕРЕД центром группы и несёт ту же мысль в простейшем виде — на множествах, без всякой алгебры.

> поле:mn ⚠ **Источник — не тот, что назван в ТЗ.** ТЗ адресовало материал в `DOKAZATELSTVA-L1/01-liniya-A.md`; там его нет (четыре раздела: ретракт, центр группы, окружность, Брауэр, слова «инъекц» ни раза). Взято из `KONSPEKT-L1/06-ne-prodolzhaetsya.md`, Утверждение 75 — там полное доказательство, — и `UPRAZHNENIYA-L1.md` §4 «Инъекции в фиксированную палитру».

> поле:mn **Реплика (не слайд).** Инъекции $X\hookrightarrow\{1,\dots,k\}$ — ровно пятый пункт «Примеров видов» из части 1: там зал видел их красивой конструкцией на биекциях. Здесь та же конструкция оказывается первым местом, где язык что-то ЗАПРЕЩАЕТ. Красивое на биекциях и невозможное на всех отображениях — один и тот же вид. Сказать вслух обязательно, иначе зал вынесет мораль «надо было сузить класс стрелок».

**Утверждение.** Сопоставление $X\mapsto\mathrm{Inj}_k(X)$ — инъекции $X\to\{1,\dots,k\}$ — не продолжается до функтора $\mathbf{FinSet}\to\mathbf{FinSet}$: ни ковариантно, ни контравариантно

{@1-2} На биекциях правило работает: биекции $\sigma\colon X\to Y$ отвечает $f\mapsto f\circ\sigma^{-1}$

{@2} $\lvert A\rvert=k$, $\lvert B\rvert=k+1$: тогда $A$ — ретракт $B$, сечение $s$ — вложение, ретракция $r$ — тождество на образе $s$

{@2} $\lvert\mathrm{Inj}_k(A)\rvert=k!$, а $\mathrm{Inj}_k(B)=\varnothing$
...
> поле:mn **Раскладка.** Крупная горизонтальная иллюстрация снизу: коммутативная диаграмма $\mathbb Z_2\to S_3\to\mathbb Z_2$ и второй этаж под ней; текстовая область над ней (правка 20.2).

> поле:mn 🔴 **Формулировка пометки переписана 30.07 (заход kod_revizia.md), и это починка, а не косметика.** Стояло «текстовая область — на всю ширину, коммутативная диаграмма ложится горизонтальной полосой снизу». `archetype_lenty` ищет подстроку «крупная горизонтальная иллюстрация снизу» для архетипа `илл-полосой-снизу`, а подстроку «на всю ширину» — для `лестница-во-всю-ширину`, у которого панелей НЕТ ВОВСЕ. Совпадала вторая, и слайд получал раскладку без единой панели: SVG диаграммы порождался в `illustrations/`, а `data-ill` на него не выставлялся. **Диаграммы на слайде не было** (`git show HEAD:src/slides/s14.html | grep -c data-ill` → 0), хотя §13 требовал ровно её («слайд превращается из текста в ДИАГРАММУ с минимальной подписью») и заход `kod_final` отчитался пунктом ✅. Ловилось это только мягким ⚠ линтера «illustrations/… не используется».
