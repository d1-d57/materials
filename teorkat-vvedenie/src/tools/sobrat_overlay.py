...
#
# 🔴 `:only-child` — НЕ УКРАШЕНИЕ СЕЛЕКТОРА, а условие, без которого правило врёт.
# Гасить маркер верно ровно тогда, когда пункт ЦЕЛИКОМ приходит на своей сцене, то
# есть сценовый спан — единственный потомок `<li>`. На «Трёх биекциях» пункт устроен
# иначе: `- $\\sum_k\\binom nk=${@3|$2^n$}` — левая часть равенства стоит с ПЕРВОЙ
# сцены, а по клику приходит только ответ. Прежний селектор считал такой пункт
# сценовым и гасил ему маркер: на слайде висели три формулы и ОДИН булет из трёх.
# Поймано глазом на кадре, гейтами не ловится (CSS верен, HTML верен, промер 0).
cascade_marker = _cascade(
    ".scene-%d .tlist li:has(> [data-scene-from=\"%d\"]:only-child)::before")
...
/* акцент: accent_tag=span, то есть **x** → <span class="acc">x</span>.
   🔴 ЧЁРНЫЙ ЖИРНЫЙ, НЕ ОХРА (свод З1/З2, владелец 31.07). Было
   `color:var(--mustard)`, и золото несло КАЖДЫЙ инлайн-акцент ленты — то есть
   попутные слова «сопоставляет» и «красиво» выделялись ровно так же сильно, как
   вводимый термин, а сам заголовок не выделялся никак: «золотым выделены часто
   вещи не очень важные, а заголовок никак не выделен». Золото теперь редкое и
   значит важность: заголовок раздела и слова-ярлыки (`.op-utv`). Термин держит
   вес, но не цвет — жирность на бумаге и так читается.
   Снимать `**` со «сопоставляет»/«красиво» отдельными правками ленты не нужно и
   не следует: цвет с них уходит этим же правилом, а жирность там уместна. */
.acc{font-weight:700;color:var(--ink)}
.acc .katex,.acc .katex *{color:var(--ink)}
...
/* ЛЕСТНИЦА РАВЕНСТВ — тот же булет, другой носитель. Строка лестницы приходит из
   `porodit.table_to_list` как `<span class="lst">`, а не `<li>`: восемь `<li>` на
   одной сцене валят линтер `build_deck` (проверка 8, порог 4 блока на клик), а
   владелец 01.08 потребовал у лестницы именно булеты — «использую такие же булеты».
   Правила ниже — построчная копия `.tlist li` выше, и это намеренно: булет обязан
   быть ТЕМ ЖЕ, иначе на слайде рядом окажутся два разных маркера. Расхождение
   единственное — межстрочный шаг задаёт `line-height` абзаца, а не `margin-top`.

   Тот же носитель несёт и обычные списки, целиком приходящие на одну сцену
   (`porodit.spisok_odnoj_sceny` → `<p class="spisok">`): у `<li>` сцену пометить
   нечем — инлайновый тег вешает атрибут на вложенный спан, и линтер считает такой
   пункт пришедшим на первую сцену. Поэтому правило общее для обоих классов. */
.lestnica .lst,.spisok .lst{display:block;position:relative;padding-left:1.15em;
  break-inside:avoid}
.lestnica .lst::before,.spisok .lst::before{content:'';position:absolute;left:0;
  top:.58em;width:.3em;height:.3em;background:var(--steel)}
/* межпунктовый шаг у списка-абзаца — тот же 10px, что у `.tlist li + li` */
.spisok .lst + .lst{margin-top:10px}

/* БУЛЕТ-РАЗДЕЛИТЕЛЬ ВНУТРИ СТРОКИ. Тот же квадрат, что маркер списка, но между
   элементами ОДНОЙ строки, а не перед строками столбика. Заведён 01.08 по прямому
   слову владельца про «Три биекции»: три тождества стоят в строку, «просто между
   ними будет разделитель в виде булета». Столбик там он отверг сам, посмотрев на
   кадр, — и правильно: три формулы в строку читаются как один ряд однотипных
   утверждений, а столбиком превращаются в перечень с порядком, которого нет.
   Размер и цвет — те же, что у `.tlist li::before`, иначе на слайде рядом окажутся
   два разных булета; `vertical-align` ставит его по оптической середине строки. */
.bul{display:inline-block;width:.28em;height:.28em;background:var(--steel);
  margin:0 .72em;vertical-align:.24em}
...
/* 🔴 КЕГЛЬ ЯРЛЫКА ПОДНЯТ 0.62 → 0.80em (владелец 01.08, глядя на слайд): «эти
   золотые заголовки здесь очень плохо видны… давай сделаем заголовок больше».
   Причина видимости не в цвете, а в размере: золото `--mustard` на бумаге
   `--paper` — контраст низкий по построению (обе тёплые и светлые), и на 0.62em
   при теле 35px ярлык шёл 22px, то есть ВДВОЕ мельче текста, который он
   возглавляет. На 0.80em это 28px. Цвет не трогаю: он утверждён вариантом «Б». */
.op-def,.op-utv,.op-task{font-family:'Forum',serif;text-transform:uppercase;
  letter-spacing:.055em;line-height:1.12;font-size:.80em}
...
/* 🔴 ЗОЛОТО (свод З2): «Утверждение» и «Упражнение» — единственные слова-ярлыки,
   которым владелец золото оставил, и оформлены они обязаны быть ОДИНАКОВО. Было
   врозь: «Утверждение» шло сталью через этот класс, а «Упражнение» — золотом, но
   инлайн-акцентом (`.acc`), потому что в `OP_CLS` его не было; на слайде «Чётные и
   нечётные» два ярлыка стояли в шести строках друг от друга и выглядели как разные
   сущности. Теперь оба идут `op-utv` (porodit.py, OP_CLS) и оба золотые.
   `--steel` тут больше не при чём, но сам токен жив: им крашены `.tlist li::before`
   и приглушённые подписи фигур. */
.op-utv{color:var(--mustard)}
...
/* маркер пункта живёт и гаснет вместе со своим пунктом (см. комментарий в скрипте).
   `:only-child` обязателен: гасить маркер можно, лишь когда сценовый спан — ВЕСЬ
   пункт. Пункт, у которого по клику приходит только хвост, виден с первой сцены,
   и булет ему нужен с первой сцены. */
.tlist li:has(> [data-scene-from]:only-child)::before{opacity:0;visibility:hidden;
