...
def build(slides):
    ...
    for sid, s in zip(ids, slides):
        ...
        if arch == "рейка-справа":
            ...
            css.append("#%s .board{grid-area:4/1/5/4;background:var(--board);"
                       "position:relative}" % sid)
            ...
            for si, sl in enumerate(zan):
                ...
                for k in sl:
                    ...
                    # `.board` теперь начинается с колонки 1 (полоса до края), а
                    # панели по-прежнему меряются по ширине ТЕКСТОВОЙ колонки
                    # (`bw` из `text_w`) — сдвиг на `left` возвращает их под текст,
                    # не под левый обрез страницы.
                    css.append("#%s .p%d{position:absolute;left:%dpx;top:%dpx;"
                               "width:%dpx;height:%dpx}"
                               % (sid, k + 1,
                                  round(left + si * (bw + 24) + (bw - pw) / 2),
