...
%% ТРИ ДВИЖКА, ОДИН ФАЙЛ. Развилка нужна потому, что листок живёт в двух средах
%% сразу: у нас он собирается LuaLaTeX (только он умеет microtype целиком —
%% растяжение глифов и трекинг, которыми строка подбирается сама), а у коллег,
%% которым .tex отдают на просмотр, стоит обычный TeX Live с pdfLaTeX и
%% Computer Modern — тем самым набором, которым листки собирались в прошлом
%% году. Файл обязан собираться и там, и там, поэтому преамбула ветвится,
%% а не выбирается раз навсегда (решение владельца 04.09).
\usepackage{iftex}
\ifPDFTeX
  %% Ветка «как в прошлом году»: T2A + babel + Computer Modern, шрифт системный.
  %% Именно так собран архив 179 школы 2025/26 (в его PDF зашиты CMR10 и
  %% cm-super: SFRM/SFTI/SFBX/SFXC). В нашей песочнице она не проверяется —
  %% кириллических пакетов для pdfLaTeX здесь нет вовсе, — поэтому ветка
  %% написана по эталонному PDF, а не по прогону.
...
\centerline{\large\textbf{16$A$. \textsc{Деревья}}}\par\medskip
\begin{samepage}
