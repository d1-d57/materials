/* ГЕЙТ СТРАНИЦЫ Л4 — запускать после КАЖДОЙ правки:
 *     node L4/check_page.js L4/L4-razbory.html
 *
...
if(!TABS){ console.log('✗ ГЕЙТ СЛЕП: TABS не достался — проверка вкладок НЕ выполнена'); process.exit(1); }
{
  ...
  // вкладка 17: переключатель ранга
  try { const t=TABS.find(x=>x.num===17); t.n=4; t.mount();
    
    console.log('  ✓ вкладка 17: ранг 4 отрисовался'); }
  catch(e){ errors.push('вкладка 17 ранг: '+e.message); console.log('  ✗ вкладка 17: '+e.message); }
}
setTimeout(()=>{
